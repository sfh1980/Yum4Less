...
**Beta v1** accepts continental US ZIP codes and browser geolocation. The map and store context work broadly; **ranked meal estimates** use **Kroger-family banners, Aldi, Publix, Food Lion, Walmart, Target, and Whole Foods** when daily ingest and promotion gates pass. **Dollar General** and **Lidl** dinner totals use a weekly ad for the ZIP, not that store. Dollar General totals show only when it is the main grocery stop nearby. Tier C — map/context only — is normal when floors do not pass, and for BJ's and other chains without a sale feed.
...
**v1 production-ranked chains:** Kroger-family banners (official API + weekly-ad fallback; Harris Teeter stays Harris Teeter), Aldi, Publix, Food Lion, Walmart, Target, and Whole Foods when promotion gates pass. Same floors for every ranked banner. Dollar General and Lidl use a weekly ad for the ZIP, not that store; Dollar General dinner totals only when no other ranked grocer is nearby. BJ's and other chains without a sale feed remain map context only.

**Not shipped:** user accounts, live checkout prices, cross-device Saved persistence. Cuisine chips (R11) **shipped** (curated + hide-empty; needs live `033`). Device-local Saved and Section H tab restyle **are** shipped. **Shipped on hardware:** TrueNAS Custom App (`db`+`app`+`ingest`), Watchtower, Cloudflare Tunnel at [`https://yum4less.com`](https://yum4less.com) — see [`docs/homelab-deploy.md`](docs/homelab-deploy.md).

Current snapshot and gaps → [`PROJECT_CONTINUITY.md` → Resume](PROJECT_CONTINUITY.md#resume-as-of-2026-09-22).
...
Runnable **beta v1** on TrueNAS with daily ingest path + Postgres — **public HTTPS live** at `https://yum4less.com` (Cloudflare Tunnel). Unattended 3am cron and backup drill **closed**. Extra ZIP `23220` first ingest is the next 3am worker. **Redesign slices 1–5 + D1–D7** shipped; **cross-device Saved** still deferred; cuisine chips (R11) **shipped** 2026-09-15.
