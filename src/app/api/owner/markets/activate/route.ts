...
export async function POST(request: Request) {
  ...
  try {
    const activated = await activateOwnerIngestMarket(parsedZip.zipCode);
    if (!activated.ok) {
      return NextResponse.json({ ok: false, error: activated.error }, { status: 400 });
    }
    ...
    return publicApiErrorResponse(
      "api.owner.markets.activate.POST",
      error,
      "That ZIP could not be activated.",
    );
  }
}
