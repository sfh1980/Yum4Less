...
const TRACKED_INGREDIENTS = PROVIDER_TRACKED_INGREDIENTS;
const SAMPLE_TRACKED = PROVIDER_TRACKED_INGREDIENTS.slice(0, 5);
...
function buildPreview(
  ...
  return {
    ...
    totalTrackedIngredients: INTERNAL_CATALOG_INGREDIENT_IDS.length,
...
describe("buildProviderCoverageRollup", () => {
  it("returns a not-available rollup when no preview exists", () => {
    const rollup = buildProviderCoverageRollup([], "none", TRACKED_INGREDIENTS);
    ...
    expect(rollup.ingredientSummaries).toHaveLength(INTERNAL_CATALOG_INGREDIENT_IDS.length);
  ...
  it("closes the trust gate when preview coverage is weak", () => {
    ...
    expect(rollup.unmatchedIngredientCount).toBe(
      INTERNAL_CATALOG_INGREDIENT_IDS.length - 1,
    );
