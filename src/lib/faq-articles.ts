...
export const FAQ_ARTICLES: readonly FaqArticle[] = [
  ...
  },
  {
    ...
    paragraphs: [
      ...
      "Dinner estimates can use Kroger-family banners (a Harris Teeter still shows as Harris Teeter), Aldi, Publix, Food Lion, Walmart, Target, and Whole Foods when we have recent sale prices for that store. Dollar General and Lidl prices come from a weekly ad for the ZIP, not that store. Dollar General dinner totals show only when it is the main grocery store nearby. If we cannot get usable sale data, that store stays on the map and we say so. Other pins without a sale feed are nearby context only.",
