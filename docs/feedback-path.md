...
- `GET /api/owner/store-coverage` (read-only storefront checklist). Search **ZIP** (Census shape) or city/state letters. TrueNAS has **`026`** (2026-08-27 00:04Z). A missing schema on other volumes returns a Coverage-tab notice and does not lock the rest of the console.
- `GET /api/owner/markets`, `POST /api/owner/markets/preview`, `POST /api/owner/markets/activate` (type a 5-digit ZIP, Check, then Activate into `active_markets`). Check ZIP shows stores **inside this ZIP shape** vs grocery pins in the **8-mile shopper circle** that sit in other ZIPs, plus which chain tools we already have. Markets also shows a zoomable coverage-area map under Active and paused (shaded shapes only, no ZIP labels; not on Coverage). Coverage search lists tracked banners first; a banner click lists that banner’s matching stores. Invalid ZIP, geocode failure, and locations outside the lower 48 are refused. A short store list is a first look, not a full catalog. ZIP `23111` is allowed if you type it. CLI backup: `npm run markets:activate -- <ZIP>`. Operator checklist: [`docs/owner-zip-intake.md`](owner-zip-intake.md).
- `POST /api/owner/store-identity/match` (optional **Link obvious twins** after Check). Writes **provisional** OSM↔official same-building links. Auto-confirm stays off. Leftover unsure pairs stay for you.
