...
| Watchtower first hourly scan | **Closed** (owner 2026-09-01 paste) — 2026-08-31 22:55Z `Updated=2` (`72c6591` / `bfe5bf4`); later hourly sessions `Updated=0`. **2026-09-03 23:55Z** `Updated=2` — app `73645334fbd8`, ingest `1aa2eb2dec54` (owner migrate `030`/`031` after pull). **2026-09-04 00:55Z** `Updated=2` — app `32bf2c300872`, ingest `f1c6fa510914` (`ec84286` store-list omit + GM junk). |
...
**Owner console (2026-08-06; ingredient review 2026-08-22; create-id 2026-08-24; tabs 2026-08-25; junk heal 2026-08-25; Coverage 2026-08-26; Markets 2026-08-27; Clear obvious 2026-09-14, merch-class junk 2026-09-16; ZIP intake + Slice D 2026-09-22; Markets ZCTA map 2026-09-22, clip 2026-09-23):** after Watchtower pulls an image that includes `/owner`, open `https://yum4less.com/owner` and paste `YUM4LESS_FEEDBACK_ADMIN_KEY`. Tabs: **Ingredient review**, **User feedback**, **Analytics**, **Coverage**, **Markets**. Coverage is read-only over `store_coverage`: Search lists tracked banners (name, ZIP, usable-in-app); a banner click lists that banner’s stores. Markets Check lists stores inside this ZIP’s shape vs 8-mile other-ZIP neighbors plus chain tools; Activate still only books the ZIP. Markets also shows a zoomable coverage-area map under Active and paused (shaded shapes only, no ZIP labels; not on Coverage). Optional **Link obvious twins** writes Slice D **provisional** OSM↔official aliases (AUTO_CONFIRM off). Operator checklist: [`docs/owner-zip-intake.md`](owner-zip-intake.md). TrueNAS has **`024`**, **`025`**, and **`026`** (`025`/`026` applied **2026-08-27 00:04Z**). Missing `026` on other volumes shows a notice on that tab only. Yes can map a flyer line to an existing food id or **create** a kebab-case id with name + category (see [`docs/feedback-path.md`](feedback-path.md)). Ingredient review shows a **Food / Junk / Unsure** preview. **Clear obvious** files leftover grocery the same way ingest will: merch classes and snack/drink/pharmacy phrases → No; short dinner foods → Yes. Live ingest skips junk and heals matching pending rows; `npm run owner:reject-pending-junk-reviews` is the one-shot for existing queues. Not linked from shopper nav. Curl still works for `GET /api/feedback` and `GET /api/analytics/events` with the same key. Image pull alone does **not** run migrations. Future `db/init` files still need an operator migrate.
