...
- **Market admission (live drain 2026-08-30):** Shopper wizard is still GPS pin or ZIP → map pin. Live ZIP `23111` ingest used **9 mi ∩ ZIP outline**, **7** weekly-ad storefronts, one flyer fetch per banner, then fan-out. Hash skip skipped unchanged Kroger/Aldi/Walmart rewrites. Manual `ingest:worker` **19:50–19:58Z** drained **6** jobs; `[freshness] OK` **361/361**. Freshness row count is lower than the pre-fence **680/681** because fewer storefronts sit inside 9 mi — not a failed night. No Lidl flyer this ZIP. Walmart hash-skip left **9** foods at **12.9h**. TheMealDB still logs **dev test key**. Do **not** claim more dinners. Extra ZIP `23220` later. **Cron Job must be Enabled** or queued 3am work sits until a hand drain.
...
- **Homelab still open (do not fold into closed):** none for ingest/backup. Watchtower first hourly scan still unconfirmed in the punch-list. Extra ZIP `23220` waits on an unattended worker night.
...
|------|----------|--------|
| 2026-08-31 | **Post-v1 shopper store gate (direction, not shipped):** Stop treating TypeScript `SHOPPER_RANKED_V1_CHAINS` as the permanent shopper allowlist. Durable store = `stores` row. Map can show more. Settings should follow `chain_registry` (and grocery pins in radius), not a hardcoded six-chain list. Dinner **estimates** still require fresh sale/price floors — that is coverage honesty, not chain favoritism. Kroger official API stays an extra **capability** when present. Dual-run CI remains until market-search/recommendations send registry flags. No new ranked adapters in the backup-drill slice. | **Active** (supersedes “ranking lists stay TypeScript until cutover” as the *goal*; dual-run still ships today) |
...
| 2026-08-30 | **First live ingest-worker drain (manual):** TrueNAS `yum4less-ingest` ran `npm run ingest:worker` and drained 6 jobs with `[freshness] OK`. Fence 9 mi ∩ ZIP. Not an unattended Cron Job proof. Keep the TrueNAS worker Cron Job Enabled while 3am only enqueues. Extra ZIP `23220` still waits. | **Active** |
...
| 2026-08-27 | **Registry dual-run (not DB-wins in the browser):** `SHOPPER_RANKED_V1_CHAINS` remains the client Settings allowlist **until the 2026-08-31 cutover**. CI asserts `chain_registry.shopper_ranked` matches that list after `027`/`028`. | **Active** (narrowed 2026-08-31: DB-wins is the goal; dual-run still ships today) |
...
| TrueNAS `029` + ingest worker | 2026-08-30 | **Manual drain succeeded.** Unattended Cron Job night still open. |
