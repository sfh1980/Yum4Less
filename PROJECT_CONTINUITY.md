...
**Phase:** Redesign **slices 1–5**, shell **D1–D7**, and post-audit hardening **Sprints A–E** **shipped**. **DB migration ledger (backlog #3) CLOSED** (2026-07-09). **Store-ID integrity bundle (#14–15) CLOSED** (2026-07-09). **Coverage ingest slices 2–5 CLOSED** (2026-07-09) — Kroger p2 terms (`020`), Flipp supplemental for unmatched ingredients, per-chain weekly-ad dedupe, Aldi Flipp+direct-scrape merge on `b9c4a46`; live re-ingest + `sync:provider-prices` on `yum4less_dev` (see [Phase 2a](#verification-snapshot)). **Chain coverage honesty (#13 + #16) CLOSED** (2026-07-09) — Track B UI (trust banner depth, Settings multi-store live summary, results skew heads-up); architectural asymmetry documented; **not** a coverage-gap fix. Aldi weekly-ad ceiling **re-confirmed** with shopper-facing explanation. **Publix weekly-ad ingest exclusion (0/97) CLOSED** (2026-07-09) — **`c18f99e`**. **Geolocation denial asymmetry (P1-3) CLOSED** (2026-07-10) — **`295daee`**. **Option A Slice 1–3 CLOSED** (2026-07-10) — identity infra + rank/pantry expand + Kroger Mechanicsville seed (`022`) + Settings canonicalize; master flag still **OFF** by default (including dev). **Active queue:** Option A **Slice 4** (Aldi↔OSM link); Settings gate bypass (P1-1); **backlog #18** — Dollar Tree / Dollar General onboarding investigation. **Backlog #5 (`tsc` bucket) CLOSED** (2026-07-09) — 0 errors + CI `npm run typecheck`.
...
- **Shipped:** `db/init/022_seed_kroger_mechanicsville_identity.sql` — seeded confirm (`match_method=seeded`, canonical `kroger-02900529`, alias `kroger-mechanicsville`); Settings canonicalize at hydrate/autosave/save/rank-resync/market-rehydrate/UI selection behind master expand flag; narrow client known-pair lookup (`store-identity-settings-lookup.ts`); `NEXT_PUBLIC_YUM4LESS_STORE_IDENTITY_EXPAND` accepted for browser bundle.
...
| **General locator-vs-OSM dedupe across all v1 chains (Option A)** | **IN PROGRESS** — Slice 1–3 **CLOSED** (2026-07-10); Slice 4 next | Alias-graph design accepted (Q1–Q3). **Slice 3:** Kroger Mechanicsville seeded (`022`) + Settings canonicalize behind flag (still **OFF**). **Named debt:** Settings known-pair lookup is Kroger-only — revisit before/during Slice 4. **Structural:** no-pointer perfect twins land on confirmThreshold 0.85 by design. **Not yet:** Aldi↔OSM; map/market-search/ingest; onboarding checklist (Slice 6). |
...
| 4 | Universal store reconciliation | **Slice 1–3 CLOSED** — Kroger seed + Settings canonicalize OFF-by-default; Slice 4 (Aldi) next; Settings known-pair debt logged | Med | Low–Med | Option A slices |
