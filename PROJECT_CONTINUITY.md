...
**Hosting:** Self-hosted TrueNAS SCALE — **Custom App `yum4less` (`db`+`app`+`ingest`) healthy**; **Watchtower** sibling up; **Cloudflare Tunnel** live at **`https://yum4less.com/`** (LAN `:3000` remains). Images on **`:homelab`**. Wizard shopper UI is on **`master`**. **Do not publish / Watchtower this market-admission slice** until TrueNAS migrate `029` + worker job (see §10.2.1). Rollback: **`backup/settings-ui-plus-catalog`** (`38bad3a`, Settings-first + catalog) or older **`backup/pre-catalog-expansion-live`** / **`:800c450`**. Watchtower still does **not** migrate by itself. Live ledger includes **`024`** (`applied_at` **2026-08-24 07:00:02Z**), **`025`/`026`** (**2026-08-27 00:04:01Z**), **`027`/`028`** (**2026-08-28 07:00:02–03Z**, applied by ingest migrate). **`029` is not on live.** Future `db/init` files still need ingest migrate or `npm run db:migrate`.
...
**Nationwide de-hardcoding:** **A** (no silent `23111`), **B1** (`active_markets` + `zip_geocode_cache` / `025`), **B2** (`chain_registry` + `store_coverage` + `/owner` Coverage / `026`), and **C** (`/owner` Markets ZIP check + activate) are **current**. **Market admission** (density class, ZCTA ∩ density ingest fence, Check inventory, flyer content-hash skip, `ingest_jobs`) is in the **local tree** (`029` on `yum4less_dev` this session). **Not on TrueNAS.** Plan → [`docs/audits/de-hardcoding-nationwide-db-driven-plan-2026-08-12.md`](docs/audits/de-hardcoding-nationwide-db-driven-plan-2026-08-12.md). Expansion is more owner-activated ZIPs. **Owner-chosen ranked ingest anchor is ZIP `23111`** (explicit `active_markets` row — not a code default). Ranked dinners still only where Kroger-family banners, Aldi, Publix, Food Lion, Lidl, or Walmart exist **and** weekly-ad promotion gates pass. Weekly-ad ingest **fail-soft** on Dollar General flyer errors; those six ranked banners stay fail-loud. Chain go/no-go stays later (free Flipp/OSM/scrape only; trial and error). Shopper ranking lists stay TypeScript with a dual-run CI check that `chain_registry.shopper_ranked` matches `SHOPPER_RANKED_V1_CHAINS` (`027` Lidl, `028` Walmart). **Live ingest overlay is off** (owner paste-back **2026-08-29**: `YUM4LESS_INGEST_ZIPS` unset). Cron visits **1 ZIP** from `active_markets` (`23111`, `source=ops`). Two unattended 3am nights EXIT completed with overlay off. Extra ZIP **`23220`** waits until `23111` looks right **after** live `029` + a TrueNAS worker — do not Watchtower this image until then. Leave `YUM4LESS_INGEST_QUEUE_WORKER` unset on live so 3am still runs the inline pipeline.
...
- **Market admission (local, 2026-08-29):** Shopper wizard is still GPS pin or ZIP → map pin; radius around **that pin**. `/owner` Check ZIP is an inventory: omits convenience/bakeries; groups will-ingest / food-only / Needs you (Target, Giant, unknown — no new adapters). Ingest fence = Census ZCTA ∩ density miles+1 (packed 5 / urban 8 / suburban 15 / rural 25, plus 1; village = rural; first night with no class = urban 9). Class saved on Activate; auto-widen only. Activate books the ZIP even if flyer tests time out. MUST = grocery in fence. Empty flyer → that banner does not rank; night can still succeed. One flyer fetch per banner **globally**, then fan-out; persist skips when content hash matches. `ingest_jobs` + `npm run ingest:worker` exist; **do not** set `YUM4LESS_INGEST_QUEUE_WORKER=1` on yum4less.com until the TrueNAS worker job exists. **`029` is not on live.** Extra ZIP `23220` later.
...
- **Settings store dropdown:** `settings-store-selection.ts` — Kroger, Aldi, Publix, Food Lion, Lidl, and Walmart always listed for selection (not gated on `recommendationEnabled`); prefers non-OSM catalog rows; **excludes `fixture-osm-*` / legacy `osm-node-90000*`**; live OSM kept only when no same-chain catalog pin within 1.5 mi; **same-chain collocated catalog twins collapsed** via `collapseSameChainCollocatedCatalogStores` (0.05 mi default; Kroger 0.15 mi exception); auto market search on Settings when setup incomplete
...
- **Homelab still open (do not fold into closed):** backup/restore drill on TrueNAS target
...
**Next (nationwide de-hardcoding — ordered):** ~~A no silent ZIP~~ **done** · ~~B1 `active_markets` + ZIP cache~~ **done** (TrueNAS `025`) · ~~B2 `chain_registry` + Coverage~~ **done** (TrueNAS `026`) · ~~C `/owner` Markets ZIP activate~~ **done** · ~~weekly-ad fail-soft for unranked chains~~ **done** · ~~Lidl/Walmart shopper-ranked v1~~ **done** (TrueNAS `027`+`028` applied 2026-08-28 07:00Z). **Market admission code is local** (`029` on `yum4less_dev`). Before Watchtower: TrueNAS migrate `029` + `ingest:worker` job; keep `YUM4LESS_INGEST_QUEUE_WORKER` unset until then. Extra ZIP `23220` after `23111` looks right on live. Organic shopper-ZIP waiting list, threshold profiles, scrape TZ, Slice D identity, `chain_registry` DB-wins in the browser, and new ranked adapters stay later.

**Later (when reprioritized):** chain go/no-go on the free stack (OSM / Flipp / scrape; no paid aggregator); Section H restyle of Home/Deals/Cook/Saved + Q34/Q35 compact Settings; backup/restore drill on TrueNAS; GYAM Cloudflare hostname on same tunnel; Saved tab persistence; cuisine chips (R11); optional SNAP CSV ingest; true client-side ranking from API-sent chain ids (dual-run CI check is current; DB-wins in the browser is later).
...
**Limits:** Shopper wizard still GPS or ZIP → pin; no H3. Census TIGERweb is skipped in `NODE_ENV=test` unless `YUM4LESS_ZCTA_LIVE=1`. **`029` is not on TrueNAS.** Do not publish / Watchtower until migrate + worker. Leave the queue env unset on live so 3am stays inline. Extra ZIP `23220` later. One Flipp ZIP fans out to all admitted storefronts — far-apart markets later need per-market flyers. No Target/pharmacy adapters. 24h freshness SLA unchanged.
...
| 2026-08-27 | **Registry dual-run (not DB-wins in the browser):** `SHOPPER_RANKED_V1_CHAINS` remains the client Settings allowlist. CI asserts `chain_registry.shopper_ranked` matches that list after `027`/`028`. True client DB-wins needs API-sent chain ids later. | **Active** (narrows “ranking lists stay TypeScript until cutover”) |
...
| 2026-08-26 | **Nationwide A / B1 / B2 current on TrueNAS:** live `schema_migrations` has `025` and `026` (`applied_at` 2026-08-27 00:04:01Z). Phase **C** shipped 2026-08-27 as owner Markets ZIP activate (not organic paused candidates). Keep ingest ZIP overlay until a market is activated. Shopper ranking stays TypeScript until a later dual-run cutover. Do not flip Walmart ranked. | **Active** (narrowed 2026-08-27: C is owner activate, not organic queue) |
...
| 2026-08-26 | **Phase B2 coverage ledger shipped:** `chain_registry` + `store_coverage` view + `/owner` Coverage tab. Search by store name, city/state, and usable-in-app (`recipe_ready`). Read-only. Same admin-key gate. Not a shopper surface. Ranking policy remains TypeScript until dual-run cutover. | **Active** (implements 2026-08-26 Coverage tab plan lock; TrueNAS `026` applied) |
...
|------|---------------|--------|
| `npm test` (market admission: density, ZCTA PIP, Check groups, flyer hash, format split) | 2026-08-29 | **1210/1210** pass (213 files) |
...
| `npm run build` (owner panel no longer bundles `pg`) | 2026-08-29 | **Pass** (Next.js 15.5.21) after `owner-market-preview-format.ts` split |
...
| `npm run test:e2e:ci` (market admission) | 2026-08-29 | **Not run** — shopper flow wiring unchanged; `/owner` is key-gated |
| Semgrep MCP (market admission files) | 2026-08-29 | Scanned unrelated temp copies — **not** a passed scan of these files |
| TrueNAS `029` + ingest worker | 2026-08-29 | **Not applied / not deployed.** Do not Watchtower until migrate + `ingest:worker` job. |
