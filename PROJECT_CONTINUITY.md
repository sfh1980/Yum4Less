...
## Resume (as of 2026-09-03)

**Owner lock:** Membership cutover **shipped in code** — `chain_registry` drives who we *attempt* for dinners / fail-loud ingest; **floors stay in code**. Thin sale coverage ⇒ pin stays tracked, dinners stay off. **Lidl lock (2026-09-03):** map/context only until a store-bound sale feed exists — no ranked dinners, no store-finder scrape work. **Dollar General (2026-09-03):** Flipp-first ZIP circular ingest; directional sales can show without dinners; dinner totals only when **no other shopper-ranked grocer is nearby** and the **same** floors pass (area circular, not that building’s shelf). Banner grain later via **shared adapter keys**. Owner Check brand lists stay research. **Ingest fence:** nightly ingest admits grocery pins in the **Census ZIP outline** with a **26 mi** safety cap — same set Owner Check lists. Shopper map stays pin + radius. Neighbor-ZIP pins on that map need those ZIPs Activated too. **Next product work:** Flipp + scrape yield in `23111` (Walmart thin coverage; Target / BJ's). **Live yum4less.com Watchtower-pulled 2026-09-04 00:55Z** (app `32bf2c300872`, ingest `f1c6fa510914`; replaced `73645334` / `1aa2eb2`). Store-list omit + GM junk skip are on the box (`ec84286`). **`030`/`031`** remain applied from **2026-09-03 23:59:42Z**. Extra ZIP **`23220` first ingest landed** on the 2026-09-01 3am worker. TheMealDB **dev test key** is a known ops gap.
...
- **Settings picker omit + GM junk skip (2026-09-04, live):** Watchtower **00:55Z** `Updated=2` — app `32bf2c300872`, ingest `f1c6fa510914`. Ingest greps confirm `dining chairs` and `isRecognizedGroceryBannerPin`. Hard-refresh the shopper UI. Pending Ingredient-review junk still needs `owner:reject-pending-junk-reviews` (or the next persist ingest). Do **not** claim more dinners.
...
**Limits:** Live yum4less.com waits Watchtower of this commit, then `owner:reject-pending-junk-reviews` again. Do not claim more dinners.
