...
**Production-ranked focus:** **Kroger-family banners, Aldi, Publix, Food Lion, and Walmart** when daily ingest and promotion gates pass. **Dollar General** uses the same floors for sale collection; dinner totals only when it is the only shopper-ranked grocer nearby (food-desert gate) — directional area circular, not shelf prices. Same floors for every ranked banner. **Lidl is map context** until a store-bound sale feed exists. Missing estimates stay map context with an honest reason — not a permanent block. **Coverage leftovers:** Target and Whole Foods adapters shipped dinners-off; BJ's map/context. **Walmart yield plan closed** (Flipp + `/owner` review) — not a coupon/checkout path.
