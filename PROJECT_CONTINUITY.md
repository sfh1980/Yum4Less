...
> **Linear history + current snapshot.** Setup and commands → [`README.md`](README.md). Shopper workflows → [`docs/shopper-workflows.md`](docs/shopper-workflows.md). Agents, MCP, hooks → [`AGENTS.md`](AGENTS.md). Full pre-reorg copy (last committed before this restructure) → [`.private/PROJECT_CONTINUITY.backup-2026-06-08.md`](.private/PROJECT_CONTINUITY.backup-2026-06-08.md) (gitignored).
...
**Hosting:** Self-hosted TrueNAS SCALE — **Custom App `yum4less` (`db`+`app`+`ingest`) healthy**; **Watchtower** sibling up; **Cloudflare Tunnel** live at **`https://yum4less.com/`** (LAN `:3000` remains). Images on **`:homelab`**. Wizard shopper UI is on **`master`**. Market-admission ingest image is on the box (worker drain 2026-08-30). Rollback: **`backup/settings-ui-plus-catalog`** (`38bad3a`, Settings-first + catalog) or older **`backup/pre-catalog-expansion-live`** / **`:800c450`**. Watchtower still does **not** migrate by itself. Live ledger includes **`024`** (`applied_at` **2026-08-24 07:00:02Z**), **`025`/`026`** (**2026-08-27 00:04:01Z**), **`027`/`028`** (**2026-08-28 07:00:02–03Z**), **`029`** (**2026-08-30 07:00:03Z**), and **`030`/`031`** (**2026-09-03 23:59:42Z**). New `db/init` files apply at **3am** ingest prep; hand `npm run db:migrate` on `yum4less-ingest` if you need them sooner.

**Production-ranked focus:** **Kroger-family banners, Aldi, Publix, Food Lion, Walmart, Target, and Whole Foods** when daily ingest and promotion gates pass. **Dollar General** dinner totals only when it is the only shopper-ranked grocer nearby. **Lidl** dinner totals use a ZIP weekly ad, not that store. Same floors for every ranked banner. Missing estimates stay map context with an honest reason — not a permanent block. **BJ's** stays map/context. **Walmart yield plan closed** (Flipp + `/owner` review) — not a coupon/checkout path. Live still waits `035`/`036` migrate.
...
**Nationwide de-hardcoding:** **A** (no silent `23111`), **B1** (`active_markets` + `zip_geocode_cache` / `025`), **B2** (`chain_registry` + `store_coverage` + `/owner` Coverage / `026`), **C** (`/owner` Markets ZIP check + activate), and **membership DB-wins** (shopper estimates + ingest fail-loud read `chain_registry.shopper_ranked`) are **current** in repo. **Market admission is on live** (owner paste-back **2026-08-30**): 9 mi fence ∩ ZIP outline, one flyer per banner, content-hash skip, `ingest:worker` drained 6 jobs. Plan → [`docs/audits/de-hardcoding-nationwide-db-driven-plan-2026-08-12.md`](docs/audits/de-hardcoding-nationwide-db-driven-plan-2026-08-12.md). Expansion is more owner-activated ZIPs. **Owner-chosen ranked ingest anchor is ZIP `23111`** (explicit `active_markets` row — not a code default). Ranked dinners still only where Kroger-family banners, Aldi, Publix, Food Lion, or Walmart exist **and** weekly-ad promotion gates pass — plus Dollar General only when none of those supermarket banners are nearby and the same floors pass. Weekly-ad ingest **fail-soft** on Lidl flyer errors; those ranked banners (including Dollar General) stay fail-loud. Owner Check still warns if no supermarket banner is in the first look (Dollar General alone does not silence that warning). Chain go/no-go stays later (free Flipp/OSM/scrape only; trial and error). CI asserts every `shopper_ranked` row has a known dinner adapter (not that TypeScript equals the table). **Live ingest overlay is off**. Cron visits **`23111`** from `active_markets` (`source=ops`) plus **`23220`**. 3am enqueue + worker drain is **proven through 2026-09-14** (do not disable the worker Cron Job). Membership was the first remaining nationwide-de-hardcoding task after A/B1/B2/C; later nationwide items (`threshold_profiles`, scrape TZ, banner-row split, organic waiting list) stay later. **Slice D** (OSM↔official proximity matcher, provisional writes) is **in repo**; expand + AUTO_CONFIRM stay **OFF**.
...
- **ZIP market intake close-out (2026-09-22, local):** Owner ZIP checklist at [`docs/owner-zip-intake.md`](docs/owner-zip-intake.md). Check ZIP shows inside-shape vs 8-mile other-ZIP lists plus chain tools. Coverage search is ZIP. Analytics has a loaded-page scoreboard and collapsed sessions. Ingredient review shows Clear obvious Food/Junk/Unsure. Slice D writes **provisional** OSM↔official links (auto-confirm off; expand off). Activate still only books the ZIP. Does **not** claim more dinners. Live waits app/ingest Watchtower.

- **Mock1 tab icons (2026-09-21):** Bottom nav uses filled Home/Deals/Cook/Saved/Feedback/Settings glyphs with per-tab colors from the V3 Mock1 shopping-list crop. Labels and active wash are unchanged. Not the rest of the grocery-ledger restyle.

- **Shopper workflow SSOT (2026-09-17):** Living catalog is [`docs/shopper-workflows.md`](docs/shopper-workflows.md). Representative Playwright branches include wizard Back, Reset Preferences, manual sale-item pick, and cuisine chips (`e2e/shopper-workflow-branches.spec.ts`). Does **not** claim every combination on yum4less.com.
...
- **Wave 2 Q1 Publix map align (2026-09-15, closed):** Publix `publix-store-locator` pins are no longer `isMapContextCatalogStore`. Merge prefers locator over OSM Publix; OSM Publix suppressed when locator is nearby. Settings selection path unchanged (already selectable). Does **not** claim more dinners.
- **Bright Data MCP paused (2026-09-15):** Owner-approved MCP entry may remain in local `.cursor/mcp.json`; **usage is paused**. Free-tier probes did not add BJ’s dinner prices; Walmart PDP can show shelf prices but Flipp + existing Target RedSky paths stay preferred. Not an ingest path; paid tier not planned. See `AGENTS.md` MCP table.
- **Target weekly-ad ingest (2026-09-15, dinners off):** Locator + promotions JSON adapter registered; `032` marks Target weekly-ad eligible / ingest_only. Fail-soft. Live after ingest image + migrate. Does **not** claim Target dinners.
...
- **Store-list omit on live app (2026-09-14):** Earlier same-day recreate **19:43Z** proved `fas mart` omit; Clear obvious image is **21:43Z**. Map may still show gas/bakery/Joe’s pins. Do **not** claim more dinners.
- **Unattended 3am + worker (2026-09-14, live):** `ingest_jobs` **15** consecutive `run_date`s **2026-08-31 through 2026-09-14**, each **6/6 `succeeded`**, `first_queued` ~**07:00:03Z** (3am Eastern). Closes “does the nightly job still run?” Do not disable the worker Cron Job. Does **not** claim more dinners.
- **Obvious grocery ingest + Clear obvious (2026-09-14, code):** Persist ingest files leftover grocery with the same planner as `/owner` **Clear obvious**. Live apply that day **applied_ok=204**. Live images pulled **21:43Z** (ops close 2026-09-15). Do **not** claim more dinners.
...
- **Market admission (unattended worker 2026-08-31):** Shopper wizard is still GPS pin or ZIP → map pin. Nightly ingest admits **ZIP outline (cap 26 mi)** on the 2026-09-03 ingest image. One flyer per banner, then fan-out. Hash skip can leave weekly-ad `observed_at` from Sunday. `23220` first ingest **2026-09-01**. No Lidl flyer. TheMealDB still logs **dev test key**. Do **not** claim more dinners. **Superseded 2026-09-15:** Walmart yield plan closed (Flipp + `/owner`); Target adapter shipped dinners-off; BJ’s Flipp confirm map/context.
...
- **Redesign UX (slices 1–5 + D1–D7 + onboarding wizard on `master`):** shipped. Shopper trees and branches → [`docs/shopper-workflows.md`](docs/shopper-workflows.md). Settings-first ZIP hero is rollback-only at `backup/settings-ui-plus-catalog`.
- **Settings store dropdown:** recognized supermarket, club, and dollar banners in radius (`isRecognizedGroceryBannerPin`) plus Target/Whole Foods — omit convenience, bakeries, specialty, pharmacies, independent OSM grocery leftovers, and map fixtures. Unranked selections stay map/context (rollout note); dinner estimates still require promotion floors. Prefers non-OSM catalog rows; **excludes `fixture-osm-*` / legacy `osm-node-90000*`**; live OSM kept only when no same-key catalog pin within 1.5 mi; **same-chain collocated catalog twins collapsed** via `collapseSameChainCollocatedCatalogStores` (0.05 mi default; Kroger 0.15 mi exception); auto market search on Settings when setup incomplete
...
- v1 ranked chains when gates pass: **Kroger-family banners**, **Aldi**, **Publix**, **Food Lion**, and **Walmart**; **Dollar General** only as a food-desert/circular fallback (same floors). Lidl, BJ's, and other unsupported chains remain map/context only
...
- **Homelab still open (do not fold into closed):** TheMealDB **dev test key** on live ingest (known). Website `robots.txt` for `/owner` **closed** 2026-09-15 (`src/app/robots.ts`). Watchtower pulls **closed** (2026-08-31 22:55Z and **2026-09-03 23:55Z** `Updated=2`). Extra ZIP `23220` first ingest **closed** (2026-09-01 worker 6/6 `succeeded`; freshness **571/571**).
- **Homelab Cloudflare Tunnel WAN** — **CLOSED** (2026-08-03/04) @ `https://yum4less.com/`
- **M128/M151 scrape automation** (robots.txt checks *before weekly-ad scrape*, auto-pause on 403/WAF, automated per-chain kill switches) — **not started in code**; containers/cron/Watchtower/tunnel/backup **are** shipped. Manual owner-pause only today.
...
6. ~~**Deferred D1–D6** — 5-tab shell, theme tokens, ingredient gate/chips, map-as-link overlay, session pantry UI~~ **done (2026-06-25)** — ~~cuisine chips (R11)~~ **done (2026-09-15)** curated chips + `cuisine_tags` / hide-empty. Device-local Saved meals shipped 2026-08-20; **cross-device Saved stays paused** (needs accounts; same horizon as Redis).
7. ~~**D7 — Color/tokens port** — Theme C (dark) + Theme D (light) from `.private/tokens.css`; flat page bg; system font; light default first visit; recolor buttons/panels/nav/map~~ **done (2026-06-26)** — owner browser verify **CLOSED** 2026-08-31 (live theme toggle in use).

**Next (nationwide de-hardcoding — ordered):** ~~A no silent ZIP~~ **done** · ~~B1 `active_markets` + ZIP cache~~ **done** (TrueNAS `025`) · ~~B2 `chain_registry` + Coverage~~ **done** (TrueNAS `026`) · ~~C `/owner` Markets ZIP activate~~ **done** · ~~weekly-ad fail-soft for unranked chains~~ **done** · ~~Lidl/Walmart shopper-ranked v1~~ **done** then **Lidl demoted 2026-09-03** (`030` live) · ~~Dollar General Flipp + food-desert dinners~~ **done** (`031` live **2026-09-03 23:59Z**) · ~~membership DB-wins~~ **done** (Watchtower **2026-09-03 23:55Z**). **Market admission is on live** (unattended worker **2026-08-31** 07:01–07:09Z; ZIP-outline fence on the 2026-09-03 ingest image). **`23220` Activate** owner-clicked 2026-08-31 (first ingest **2026-09-01**). **Settings grocery-pin picker** shipped. **Slice D** catalog OSM↔official matcher is **in repo** (provisional writes; AUTO_CONFIRM and shopper expand stay **OFF**). Organic shopper-ZIP waiting list, threshold profiles, scrape TZ, banner-row split, and new ranked adapters stay later.

**Later (when reprioritized):** chain go/no-go on the free stack (OSM / Flipp / scrape; no paid aggregator) — **keep in docs, not near-term**; GYAM Cloudflare hostname on same tunnel; Saved tab **cross-device** persistence (paused with accounts); SNAP nationwide USDA CSV on TrueNAS (parser + fixture path **shipped**; live CSV path is ops-optional); banner-per-row with shared family adapters (not 20 copied Kroger clients). Owner Check fragment lists: research, stay open. New ranked adapters stay open. **Never** coupon/checkout. User accounts stay out until Redis-scale traffic. BJ's / Target **ranked** pricing stays later until floors + membership — Walmart yield plan already closed (Flipp + `/owner`). Cuisine chips (R11) **shipped 2026-09-15**.
...
**Shopper path (trees, branches, symptoms):** [`docs/shopper-workflows.md`](docs/shopper-workflows.md). This section keeps locked product decisions only.
...
Shopper tabs, Home steps, and locked-tab copy → [`docs/shopper-workflows.md`](docs/shopper-workflows.md). D1 lock: mobile-first; functional on desktop without a separate layout.

### Flow and session (shipped)

Splash, wizard, Reset Preferences, and session vs saved prefs → [`docs/shopper-workflows.md`](docs/shopper-workflows.md). Session state (`cookEnabled`, pantry) is derived, not a DB row.
...
Saved tab **cross-device** persistence stays paused (device-local Saved shipped). Cuisine chips (**R11**) **shipped 2026-09-15** (`cuisine_tags` + curated hide-empty chips). Mockup layout polish from the old Settings-first shell is closed (wizard + Section H).
...
| **D7** | ~~Mockup color/tokens port (colors only)~~ | ~~`theme-tokens.css`, `globals.css`, component CSS, `resolve-theme` default~~ | ~~`npm test`; owner browser verify; Playwright MCP for trust labels~~ **done** — owner browser verify **CLOSED** 2026-08-31 |
...
**Theme:** Whole Foods stayed ingest-only, and Coverage listed an OpenStreetMap pin and a retailer pin for the same building.

**Shipped:** `035` sets Whole Foods `shopper_ranked` with the same weekly-ad dinner gates as the other ranked banners. `036` does the same for Target. Lidl is shopper-ranked with shopper copy that the ad is for the ZIP, not that store. Dollar General uses that same plain ZIP wording, and still only totals dinners when it is the main grocery store nearby. Coverage collapses same-banner pins within 0.05 miles into the retailer row and notes the map pin is the same place. Removed `MEAL_PRICING_COMING_LATER_CHAINS`. Dinner totals follow `shopper_ranked` plus the shared sale floors.

**Limits:** The two `stores` rows stay in Postgres until a later identity merge. Live waits app image plus ingest migrate for `035` and `036`. Watchtower does not migrate by itself.

**Evidence:** `npm test` **1309/1309** (233 files) this session. Did not run `test:integration` or `test:e2e:ci`.
...
**Theme:** ZIP-generic Whole Foods circular ingest. Same adapter for any activated ZIP — locator resolves `store-id`, then the sales flyer is fetched by that id.

**Shipped:** Closest-store locator (geocode ZIP → retailer session) + unauthenticated store summary + `sales-flyer?store-id=` `__NEXT_DATA__` parser. Persist `source_store_id` / optional `whole-foods-{id}` catalog upsert. Migration `034` sets Whole Foods `weekly_ad_eligible` / `ingest_only`; **`shopper_ranked` stays false** (fail-soft). Fixture + unit tests. Owner Check treats Whole Foods as will-ingest. No hardcoded store `10598` / ZIP `23220`.

**Limits:** Dinners stay off. Flipp has no Whole Foods merchant. Live waits ingest image + `034` migrate. Percent-only flyer lines are skipped.

**Evidence:** this session `npm test` **1304/1304** (231 files). `npm run test:integration` **54/54** (17 files; applied `034`). `npm run typecheck` pass. `npm run build` pass (Next.js 15.5.25). Semgrep CLI `p/default` **0 findings** (1.165.0) on adapter/rollout/`034`. Postgres MCP after local `db:migrate`: Whole Foods `ingest_only` / `weekly_ad_eligible=true` / `shopper_ranked=false`. Did not run `test:e2e:ci` (no shopper dinner-flow wiring). Live waits ingest image + `034` migrate.
...
**Shipped:** [`docs/owner-zip-intake.md`](docs/owner-zip-intake.md). Check ZIP two lists (ZIP shape vs 8-mile neighbors) and adapter capability lines. Shared hyphen-normalized convenience omit (Dash-In / Fas-Mart). Coverage ZIP search via Census shape. Analytics loaded-page scoreboard + collapsed sessions. Clear obvious Food/Junk/Unsure preview. Slice D catalog matcher writes **provisional** aliases after map-catalog and from Markets **Link obvious twins**. AUTO_CONFIRM and shopper expand stay **OFF**. Activate still books the ZIP only.

**Limits:** No Run intake now. No neighbor-ZIP Activate chips. Matcher does not confirm links. Does not invent dinners. Live site waits Watchtower.

**Evidence:** this session `npm test` **1295/1295** (229 files). `npm run build` pass (Next.js 15.5.25). Semgrep CLI **1.165.0** on identity/coverage files: **0 findings** (OSS; unrelated Pro-engine rule warnings). Did not run e2e (owner console only; shopper flow wiring unchanged). Did not run integration (no schema change).

### 2026-09-21 — Mock1 tab icons and colors

**Theme:** Owner lock: apply V3 Mock1 shopping-list tab icons and their colors to the live 6-tab shell.

**Shipped:** Filled Home (terracotta house), Deals (amber price tag), Cook (silver fork/knife), Saved (red bookmark), Feedback (lavender bubble), Settings (purple gear). Colors are `--nav-icon-*` tokens in light and dark. Active tab still uses the existing label/wash; icons stay their Mock1 fills.

**Limits:** Not the grocery-ledger palette, price-tag badges, or running-total bar. Icons are inline SVG, not a 3D illustration set.

**Evidence:** this session `npm test` **1285/1285** (226 files). Did not run e2e (nav labels/roles unchanged). Did not run integration.

### 2026-09-17 — Shopper workflow catalog + e2e branch gaps

**Theme:** One living shopper-flow catalog; representative Playwright branches; no live yum4less.com crawl.

**Shipped:** [`docs/shopper-workflows.md`](docs/shopper-workflows.md) inferred from current planner code. Stub at `docs/application-decision-trees.md`. Flyer-line tree moved to [`docs/provider-integration-pattern.md`](docs/provider-integration-pattern.md). Duplicate trees/tables removed from the redesign locked-plan workflow section, redesign handoff, and README (history rows left). Documentation-owners tables in product-and-trust / continuity-journal rules now list the catalog. New e2e: wizard Back, Reset Preferences, manual ingredients, cuisine chips.

**Limits:** Not exhaustive combinatorial coverage. Not pointed at production.

**Evidence:** this session `npm test` **1284/1284** (226 files). `npm run test:e2e:ci` **37 passed** / **2 skipped**. `npm run build` ran inside e2e:ci (Next 15.5.25). Did not run `test:integration` (no schema/persist change). Semgrep not run (docs + e2e locators only).
...
**Theme:** Shopper dinner filter by curated cuisine; hide empty chips until enough dinners match.

**Shipped:** `cuisine_tags` on `recipes` (`033` + backfill from summary Cuisine prose). Curated chips: American, Italian, Mexican, Chinese, Thai, Japanese, Greek, Indian, Korean, Vietnamese. TheMealDB `strArea` map (British/Canadian → American). Rank/pantry pool filter via `selectedCuisineIds`. Server `cuisineFacetCounts` on market (recomputed on pass-through). Ingredients-step chip toolbar; min count **2**. Inventory R11 **closed**.

**Limits:** Live after `033` migrate + app/ingest Watchtower. Thin catalogs hide most chips until more TheMealDB imports map. Does not invent dinners or change floors.

**Evidence:** this session `npm test` **1279/1279** (226 files); `npm run test:integration` **54/54** (17 files, applied `033`); `npm run test:e2e:ci` **32 passed** / **2 skipped** (after `npx playwright install chromium`).

### 2026-09-15 — Scale risk A: client-trust API audit (closed)

**Theme:** Systematic audit of public API routes for client-controllable trust fields (beyond rank pass-through).

**Shipped:** Audit of recommendations, market-search, pantry-coverage, shopping-route, geocode, analytics, feedback, debug/pipeline. Rank + pantry already trim → rehydrate → recompute; market-search is server-built + sanitized. Added pantry pass-through spoof tests (disabled-gate and invented-store id). Marked analytics `market_data_source` / `has_fallback_notice` / `recommendation_enabled` as client-stamped non-authoritative (`CLIENT_STAMPED_ANALYTICS_TRUST_DIMS` + owner Analytics hint). Inventory Scale risk A **closed**.

**Limits:** Shopping-route still echoes client store/home labels (cosmetic). Analytics dims remain allowlisted for honest telemetry. Does not invent more dinners. Live after app Watchtower pull for owner copy.

**Evidence:** this session `npm test` **1274/1274** (225 files). Did not run `test:e2e:ci` (owner Analytics hint only; no shopper flow wiring).

### 2026-09-15 — Walmart yield plan closed (Flipp + `/owner`)

**Theme:** Close the “Walmart thin Flipp yield” product task without claiming more dinners.

**Shipped (docs/decision):** Owner lock — Flipp remains the primary Walmart circular source; retailer HTML scrape only when Flipp is empty (code already shipped). Improving usable food from junk-heavy flyers is **ops**: junk SSOT + `/owner` Ingredient review (Yes food / No skip). Inventory §2.1 and open-slice row **closed**. Not stealth browsers, residential proxies, or Bright Data.

**Limits:** Does not invent more Walmart dinners. Floors still gate estimates. `/owner` review stays ongoing human work.

**Evidence:** Owner decision this session; docs only (inventory + Decision log + Resume). No new code.

### 2026-09-15 — Wave 2 Q1 Publix map-context align (closed)

**Theme:** Honor locked Q1=1B — Publix store-locator pins are Settings-selectable catalog, not map-context peers.

**Shipped:** Removed Publix locator special-case from `isMapContextCatalogStore`. Unit tests: classification, merge priority vs OSM Publix, OSM suppress when locator is the ingested ranked peer. Docs: store-identity onboarding + open-work inventory.

**Limits:** Does not change dinner floors or invent more Publix dinners. `getCatalogStoreRole("publix-store-locator")` still returns map-context for the older source-role helper (ingest coordinate path); Settings already selected Publix via chain filters. Live after app Watchtower pull.

**Evidence:** this session `npm test` **1271/1271** (224 files). Did not run `test:e2e:ci` (no Settings/map component wiring change; e2e already covers Publix checkbox). Live after app Watchtower pull.

### 2026-09-15 — Bright Data MCP paused (docs)

**Theme:** Keep free-stack Flipp/API/scrape as coverage; park paid-adjacent Bright Data.

**Shipped (docs/ops only):** Bright Data MCP configured locally (gitignored token). Owner probes: BJ’s pages load but no usable shelf prices; Walmart PDP can return absolute prices; Target weekly-ad/deals weak vs RedSky. **Usage paused** — do not call Bright Data for ingest, adapters, or routine agent work until unpaused. Free ~5k req/mo insufficient for coverage; paid tier not planned.

**Limits:** MCP server may still show enabled in Cursor Customize; agents must treat it as off. Does not change Flipp/Walmart/Target/BJ’s product status.

**Evidence:** owner decision this session; documented in `AGENTS.md`, open-work inventory, Decision log.

### 2026-09-15 — Target weekly-ad ingest adapter (dinners off)

**Theme:** Store-bound Target circular ingest without ranked dinners.

**Shipped:** ZIP-generic RedSky `nearby_stores_v1` locator → persist `source_store_id` / optional `target-{id}` catalog upsert → `store_promotions` + `promotions/{id}` JSON parse → shared junk/matcher → `target-weekly-ad-scrape` observations. Migration `032` sets Target `weekly_ad_eligible` / `ingest_only` / adapter name; **`shopper_ranked` stays false** (fail-soft). Fixture + unit tests. Owner Check treats Target as will-ingest. No hardcoded store `1968`.

**Limits:** Guest/public web API key can rotate or PX-block. BOGO/percent-only lines skipped. Dinners stay off until floors + membership. Live after ingest Watchtower pull + `032` migrate. Does not invent more dinners.

**Evidence:** this session full battery — `npm test` **1267/1267** (223 files); `npm run test:integration` **54/54** (17 files, applied `032`); `npm run build` pass (Next 15.5.25); `npm run test:e2e:ci` **32 passed** / **2 skipped** (after `npx playwright install chromium`). Live after ingest Watchtower pull + `032` migrate.
...
**Limits:** Mid-request TheMealDB failures can still 500. Shopping-route never read Postgres (inventory example was stale) — it now returns 503 on planner throws. Scale risk A (client-trust audit) **closed** 2026-09-15. Live after app Watchtower pull.
...
**Theme:** Tell search crawlers to skip the key-gated owner console (SEO file, not scrape M128).

**Shipped:** `src/app/robots.ts` — `User-agent: *`, allow `/`, disallow `/owner` and `/api/`. No sitemap line (v1 skip). Owner page already had `robots: { index: false }`. Unit test `robots.test.ts` **1/1**. Inventory SEO leftover **closed**.

**Limits:** Live body appears after CI publish + Watchtower app pull. Cloudflare may prepend content-signal comments; app `Disallow` lines must still be present. Does not lock `/owner` (admin key still required). Not M128 scrape automation.

**Evidence:** `npx vitest run src/app/robots.test.ts` **1/1**. Owner close on commit/push this session.

### 2026-09-15 — Leftover grocery / Clear obvious live ops closed

**Theme:** Confirm the grocery-image pull and Clear obvious path on TrueNAS so the ops F task can close.

**Shipped (ops):** App + ingest `:homelab` created **2026-09-14 21:43Z** after CI publish of `d3e9396`. Live ingest has `planPendingReviewResolution`; app has **Clear obvious**. Dry-run **yes=3 no=0 skip=34**. Owner monitors `/owner` Ingredient review as needed. Inventory “leftover grocery / Clear obvious” **closed**.

**Limits:** Skip/unsure titles stay human. Nightly ingest can still add pending rows. Does not invent more dinners. TheMealDB **dev test key** remains open ops.

**Evidence:** Owner TrueNAS paste-back this session (`docker inspect` + grep + pending count + dry-run). Docs only; no new unit tests.

### 2026-09-14 — Live SQL snapshot (registry, Walmart/DG/Lidl, ledger)

**Theme:** Optional TrueNAS read of membership, thin-banner stock, and migrations.

**Shipped (ops):** Inventory “optional live SQL” **closed**. `chain_registry` matches `030`/`031` (Lidl map-context; Dollar General ranked). Walmart weekly-ad **19** in-stock / **19** in 24h (newest **2026-09-14 07:09:43Z**). Dollar General and Lidl **no** in-stock rows. `schema_migrations` **31** files **000–013, 015–031** (`version` is text; **014** is not in the repo).

**Limits:** Did not re-run the full ranked-price heartbeat. 19 Walmart rows do not mean more dinners. DG dinners stay off in `23111`.

**Evidence:** Owner TrueNAS `psql` paste-back (this session).

### 2026-09-14 — Store-list omit is on the live shopper image

**Theme:** Fas Mart / convenience omit does not need a standing hard-refresh task.

**Shipped (ops):** Live `yum4less-app:homelab` created **2026-09-14T19:43:47Z**. `fas mart` is in shopper JS (`page.js`, chunk `783.js`, static `page-45c97d3e…js`). Inventory hard-refresh Settings/Owner Check **closed**. Map pins can still show those names.

**Limits:** Did not eyeball Settings in a browser this paste. Ingest container not inspected. Does not invent more dinners.

**Evidence:** Owner TrueNAS `docker inspect` + `grep` paste-back (this session).

### 2026-09-14 — Watchtower-migrate reminder is not an open task

**Theme:** New `db/init` files do not need a standing “remember to migrate” ops item.

**Shipped (ops):** Inventory “Watchtower does not migrate” **closed** as unfinished work. 3am scheduled ingest already runs `ensure-test-db` / pending SQL. Hand `sudo docker exec yum4less-ingest npm run db:migrate` if you want it before morning. Watchtower still does not run SQL. Do not auto-reset the live volume.

**Limits:** No ingest-entrypoint auto-migrate on Watchtower recreate. Hours can pass between image pull and 3am.

**Evidence:** Owner lock this session. Docs only; no tests.

### 2026-09-14 — Local ≠ live is process, not an open task

**Theme:** Laptop Docker and yum4less.com are two databases. That is correct; do not build a sync.

**Shipped (ops):** Inventory “local `yum4less_dev` ≠ yum4less.com” **closed** as unfinished work. The 2026-08-25 lock stays: live counts only from NAS paste-back ([`docs/homelab-deploy.md`](docs/homelab-deploy.md) §4.5). No Cursor MCP to TrueNAS, no daily dump, no DB rename.

**Limits:** Agents can still mix them up if they ignore paste-back. Watchtower image lag is a separate mismatch (git ≠ the site).

**Evidence:** Owner lock this session. Docs only; no tests.

### 2026-09-14 — Unattended 3am + worker: 15-night proof

**Theme:** Confirm the nightly ingest still runs every morning without a hand start.

**Shipped (ops):** Live `ingest_jobs` for `run_date` **2026-08-31 through 2026-09-14**: **15/15** nights, **6/6 `succeeded`**, **0 failed**, `first_queued` ~**07:00:03Z** (3am Eastern). Inventory “keep proving the Cron Job” is **closed**. Do not disable the worker Cron Job.

**Limits:** Does not refresh the 2026-09-05 freshness heartbeat. Does not invent more dinners. 3am enqueue is in-container cron; TrueNAS Cron Job is the worker drain.

**Evidence:** Owner TrueNAS paste-back of the `ingest_jobs` `run_date` rollup (this session). No unit tests (docs/ops only).

### 2026-09-14 — Obvious grocery on ingest + `/owner` Clear obvious

**Theme:** Leftover flyer grocery should file itself instead of piling up on `/owner`.

**Shipped:** Live persist classify runs `planPendingReviewResolution` after junk skip, alias/fuzzy match, and simple-food auto-create. Obvious food → match or auto-create; obvious junk → skip `obvious-no`; unsure → review. `/owner` **Clear obvious** POSTs `action: clear-obvious` through `applyObviousPendingReviews`. Smashburgers, filet mignon, and baba ganoush map as foods. Grill Mates titles map to generic `grill-seasoning` before generic steak.

**Limits:** Unsure sides/mystery sauces still need a human. Does not rewrite old brand ids. Does not invent more dinners. **Live ops closed 2026-09-15:** Watchtower recreated app + ingest **2026-09-14 21:43Z**; planner + Clear obvious confirmed on the box.

**Evidence:** this session `npm test` **1246/1246** (218 files). `npm run build` pass (Next 15.5.25; `/api/owner/ingredient-reviews` + `/owner`). Targeted planner + classify + owner route + console **29/29** after garden-salad auto-create and Grill Mates → generic `grill-seasoning`. `npm run test:e2e:ci` not run (`/owner` is key-gated; shopper flow unchanged). Semgrep MCP timed out on these files — not a passed scan. Owner TrueNAS paste-back **2026-09-15:** both containers **21:43Z**; dry-run **yes=3 no=0 skip=34**.
...
**Live apply (2026-09-14, owner paste):** **yes=190 no=14 skip=27 applied_ok=204 applied_fail=0**.
...
**Theme:** Live heal dropped Halloween/GM junk to **617** pending. The dump was mostly dinner grocery plus a second wave of bottled water, energy drinks, candy, baby pouches, Oral-B, and prepared pizza.

**Shipped:** Same SSOT: HappyBaby/HappyTot one-word forms; Evian/Deer Park/Liquid Death; LunchMakers/Harvest Snaps/Nature Valley; Wild Mike’s / CPK / deli pizza (not dough); Oral-B; `toothbrush(?:es)?` so singular toothbrush matches; `frito-?lay` so hyphenated Frito-Lay matches. Do not use bare pizza, tea, pretzel, muffin, coffee, water, or drumstick (pizza dough, iced tea, Pretzilla buns, English muffins, StarKist in water, chicken drumsticks).

**Limits:** Live queue **2026-09-13:** **scanned=429 rejected=0 remaining=429** (was 617 on 2026-09-10). Leftover steaks, salads, sauces, dough, Eggo, ziti, cheddar, broth, and produce still need `/owner` Yes. Does not invent more dinners.

**Evidence:** `npx vitest run` junk + classify **10/10**. Full `npm test` **1237/1237** (217 files). CI publish-ingest-image **success** on `4a57fd4` ([34517618028](https://github.com/sfh1980/Yum4Less/actions/runs/34517618028)). Live heal **2026-09-13: scanned=429 rejected=0 remaining=429**.

### 2026-09-10 — Skip leftover Halloween / GM / drink / snack owner-queue titles

**Theme:** After the pharmacy/seltzer heal (`rejected=0`, **902** pending), the live CSV was still mostly Aldi Halloween/furniture/apparel, bottled drinks, snack crackers, baby food, frozen singles, and Walmart toys — not dinner foods.

**Shipped:** Same SSOT (`isWeeklyAdJunkProduct`): Halloween collection/plush; SOHL/Crane/Serra GM; Pokémon; dehydrator/parchment/gallon bags; croissants/breakfast bowls/energy bars; meal kits/pizza singles/frozen dinners; seltzer/PurAqua/Bodyarmor/Guinness stout; wine brands that never say wine (La Marca, Stella Rosa, Meiomi); fruit snacks/gummies/taquitos/crackers; baby food/Gerber; toys (Intex/Play Day/Little Tikes); title `Each` / laundry `GAIN`. Plural `entrées` now matches. Do not use bare pizza, tea, pad, paper, pretzel, or each-as-substring (pizza dough, iced tea, Pretzilla buns).

**Limits:** Live heal **2026-09-10:** **scanned=902 rejected=285 remaining=617**. Leftover steaks, salads, sauces, buns, Eggo, ziti, cheddar, broth, grains, and produce still need `/owner` Yes. Does not invent more Walmart food SKUs or more dinners.

**Evidence:** `npx vitest run` junk + classify **10/10**. Full `npm test` this session **1237/1237** (217 files). Live heal paste-back **scanned=902 rejected=285 remaining=617**.
...
**Theme:** Live Ingredient review still queued Advil/Zyrtec/pads, White Claw/Pepsi/Dasani, bath tissue, bouquets, cereal/protein bars, and leftover Walmart pools/cookware. None of those are dinner foods.

**Shipped:** Same SSOT (`isWeeklyAdJunkProduct`): deodorant/tampons/pharmacy brands; hard seltzer and beer/wine lines that never say beer or wine; Pepsi/7UP/Gatorade/energy drinks; purified/sparkling water brands; kombucha; K-Cup/coffee pods; cereal/granola/protein bars; lunchables/popcorn/Chex Mix; bouquets/dozen roses; cookware sets/deep fryer/Crock-Pot; above-ground pools/splash pads; pet “for dogs/cats”; Kingsford/lump charcoal. Do not use bare tea, pad, water, pizza, daisy, or rose-as-prefix (iced tea, watermelon, pizza dough, Daisy sour cream, rosemary).

**Limits:** Does not skip steaks, salads, meatballs, Eggo, ziti, Caesar kit, cheddar, broth, or Grill Mates. Live `/owner` queue stays until Watchtower pulls this ingest image, then persist ingest or `owner:reject-pending-junk-reviews`. Missing Aldi/Lidl/Food Lion A–P titles were not in the pasted slice.

**Evidence:** `npx vitest run` junk + classify **10/10**. Full `npm test` this session **1235 passed / 2 failed** (217 files): the 2 failures are live Aldi Flipp 15s timeouts, not this matcher. Do not treat as CI green.

### 2026-09-09 — Skip Walmart GM leftover chairs / baby gear / coolers / cleaners / beer brands

**Theme:** Owner Ingredient review still queued camping chairs, baby monitors, ice chests, charcoal grills, water bottles, sandwich bags, Fabuloso/Windex/Comet, and beer lines that never say “beer” (Stella, Blue Moon, Yuengling).

**Shipped:** Same SSOT (`isWeeklyAdJunkProduct`): baby monitor / playard / pack n play / layette; camping/office/tub/saucer/bean-bag chairs; coffee/end tables; chaise; ice chest / soft cooler; charcoal grill or briquettes; griddle kit; loafers / ballet flats / cami top; water bottle; sandwich bags; Windex / Fabuloso / Comet / floor cleaner; Stella Artois / Michelob / Yuengling / Twisted Tea / Busch / Natural Light / Keystone / Blue Moon. Do not use bare chair, table, grill, cooler, coffee, monitor, or baby (Grill Mates, ice cream, baby spinach).

**Limits:** Does not skip Eggo waffles, Great Value ziti, or Caesar salad kit — those stay foods (Yes onto pasta/waffles/salad as you choose). Live queue clears after Watchtower pull + persist ingest or `owner:reject-pending-junk-reviews`.

**Evidence:** `npx vitest run` junk + classify **10/10**. Full `npm test` this session hit worker-pool timeouts and 2 live Aldi Flipp 15s timeouts (1192 passed / 2 failed / 6 pool errors) — not treated as this slice.

### 2026-09-09 — Open work inventory (`docs/`)

**Theme:** Dated list of open tasks, slices, and ops so a return-from-break does not depend on chat memory.

**Shipped:** [`docs/open-work-inventory-2026-09-09.md`](docs/open-work-inventory-2026-09-09.md). Resume remains live status. GitHub issues were 0 open.

**Limits:** Snapshot of Resume/Home as of 2026-09-05 paste-back plus 2026-09-07 Target/Walmart research (not coded). Will drift if not refreshed when slices close.

**Evidence:** docs only; did not run `npm test`.

### 2026-09-07 — Database-owned data rule (no hardcoded stores/ZIPs)

**Theme:** Owner lock: backend TypeScript is behavior only. Store numbers, ingest ZIPs, chain membership, catalog pins, prices, and ingredients live in Postgres (or are resolved from the retailer and persisted). Target `1968` was the example — not a constant to bake into an adapter.

**Shipped:** Always-on rule `.cursor/rules/yum4less-db-owned-data.mdc`; session-start hook reminder; pointers in orchestration, ingest workflow, AGENTS.md, ingest/backend/database agents.

**Limits:** Does not add a Target adapter. Existing promotion floors and name-fragment lists were not migrated in this slice. Did not run `npm test` (rules/docs only).

**Evidence:** owner direction this session.

### 2026-09-05 — Live ingest health paste-back (`42c655c` on box)

**Theme:** After a few days, owner ran TrueNAS checks. Nightly ingest is healthy; leftover junk heal found nothing new; store-list omit is in the live ingest image.

**Shipped (ops):** Images created **2026-09-04T01:55:17Z** — app `55ad51dac9b6`, ingest `6b541e14fc3f`. Grep shows `isRecognizedGroceryBannerPin`, `bluetooth`, `little debbie`. `TRUST_PROXY_HEADERS=1`. `active_markets` **23111** + **23220**. Worker jobs are kind-level (not per-ZIP keys); all 42 rows in 8 days `succeeded`, empty `last_error`. Heartbeat **428/428**. Second `owner:reject-pending-junk-reviews` **0 rejected**.

**Limits:** `stores` has no `zip`/`chain_key` — SQL cannot prove Settings omit; hard-refresh the picker. Kroger weekly-ad `observed_at` can lag `last_verified_at` (same flyer, verify-touch). No Walmart/DG/Lidl in-stock ranked rows in the heartbeat. Do not claim more dinners.

**Evidence:** owner TrueNAS paste-back 2026-09-05. Did not run unit/e2e this session.

### 2026-09-04 — Skip leftover apparel / speaker / snack flyer junk

**Theme:** After live junk heal (1059 scanned / 31 rejected), the next 40 pending rows still had clothes, a Bluetooth portable speaker, routers, Keurig pods, and snack brands.

**Shipped:** Same SSOT: `bluetooth` (not only adjacent `bluetooth speaker`), hand mixer, women's-and-women's / plus apparel, track/fleece/skort, WiFi 6, water filtration, mopping cloths, Glade, mold-and-mildew, pizza rolls, cakes (plural), Hostess/Little Debbie/Stouffer's/Keurig. Do not use bare `tee` or `short` (iced tea, short ribs). Keep salad dressing, steak sauce, Caesar kit, slider rolls, Old El Paso dinner kit, Reser's sides.

**Limits:** Live pulled **2026-09-04 01:55Z**. Second junk heal **rejected=0** (remaining 971 are grocery-looking titles). Do not claim more dinners.

**Evidence:** this session `npm test` **1237/1237** (217 files).

### 2026-09-04 — Live Watchtower pull for store-list omit + GM junk (`ec84286`)

**Theme:** yum4less.com is on the grocery-banner picker and expanded junk-skip image.

**Shipped (ops):** Watchtower **2026-09-04T00:55Z** `Updated=2` — app `32bf2c300872`, ingest `f1c6fa510914` (removed `73645334` / `1aa2eb2`). Postgres Up 4 days. Site `/` `/faq` `/terms` **200**. `TRUST_PROXY_*` still `1`. Ingest source greps: `dining chairs`, `isRecognizedGroceryBannerPin`. CI [**33822665379**](https://github.com/sfh1980/Yum4Less/actions/runs/33822665379) @ `ec84286`. No migrate this pull.

**Limits:** Hard-refresh the shopper UI. Pending Ingredient-review rows stay until persist ingest or `owner:reject-pending-junk-reviews`. Do not claim more dinners.

### 2026-09-03 — Skip Walmart GM leftover flyer junk

**Theme:** Owner Ingredient review queued patio furniture, gym weights, Dyson hair tools, TVs, car seats, and cookware brands. None of those are dinner foods.

**Shipped:** Same SSOT (`isWeeklyAdJunkProduct`): dining chair/table, outdoor dining, electric skillet, food processor, ice maker, Instant Pot / multi-cooker, dumbbell/kettlebell/barbell/weight set, Dyson/Airwrap/straightener, gaming monitor, smart TV/QLED, car seat, Bluetooth speaker, fly trap / fruit flies, refurbished. Do not use bare `tv` (TV dinner), `ice` (ice cream), `table`, or `chair`.

**Limits:** Live ingest image pulled **2026-09-04 00:55Z**. A later local slice adds apparel/speaker/snack leftovers from the remaining pending sample; live queue waits for the next image + `owner:reject-pending-junk-reviews`. Do not use bare `oven`, `air fry`, `pet`, `dog`, `cat`, `cream`, `oil`, `tv`, `ice`, `tee`, or `short`.

**Evidence:** this session `npm test` **1237/1237** (217 files).

### 2026-09-03 — Omit specialty / bakery / independent grocery from store lists

**Theme:** Owner lock: do not count OSM grocery leftovers, specialty (Capt Gregs), or bakeries on Settings, Owner Check, or density. Map can still show those pins.

**Shipped:** Shared `isRecognizedGroceryBannerPin` / `isMapOnlyFoodRetailPin`. Settings picker, Check inventory, and 8-mile density counts keep known banners, clubs, dollar markets, Target, and Whole Foods. Fas Mart, Dash In, bakeries, seafood/butcher, and unnamed independents (Joe's) are omitted. Pharmacies stay Check-only (needs-you).

**Limits:** Live pulled **2026-09-04 00:55Z**. Density class may drop a notch where independents were inflating the 8-mile grocery count. Do not claim more dinners.

**Evidence:** this session `npm test` **1237/1237** (217 files). Did not run `npm run test:e2e:ci` (bare Playwright against local `:3000` never reached the splash — not a picker-filter failure).

### 2026-09-03 — Live Watchtower + `030`/`031` migrate (owner paste)

**Theme:** yum4less.com is on the Dollar General / Lidl-demotion images, and the live roster matches `chain_registry`.

**Shipped (ops):** Watchtower **2026-09-03T23:55Z** `Updated=2` — app `73645334fbd8`, ingest `1aa2eb2dec54` (removed `72c6591` / `bfe5bf4`). Postgres Up 4 days. Site `/` `/faq` `/terms` **200**. Owner `db:migrate` applied **`030`/`031`** at **23:59:42Z**. Live flags: Lidl `shopper_ranked=false` / `map_context`; Dollar General `true` / `ranked`; Walmart unchanged. CI [**33815205043**](https://github.com/sfh1980/Yum4Less/actions/runs/33815205043) @ `b3f5b20`.

**Limits:** Hard-refresh the shopper UI. `23111` still has other grocers, so DG dinners stay off. Ingredient-review junk backlog waits for persist ingest or `owner:reject-pending-junk-reviews`. Do not claim more dinners.

### 2026-09-03 — Skip countertop-oven / air-fryer flyer junk

**Theme:** Owner Ingredient review should not queue appliances. The Walmart Ninja French Door countertop oven (air fry, dishwasher-safe) was a near-miss: junk already skipped blenders/toasters, not `countertop oven` / `air fry function`.

**Shipped:** Same SSOT (`isWeeklyAdJunkProduct`): countertop/toaster oven, air fryer, air fry function, dishwasher / dishwasher-safe, French door, pressure/rice cooker, coffee maker. Pet aisle: pet food/treats/beds/supplies, dog/cat treats, cat/kitty litter, puppy chow, bird seed, fish food, flea & tick, rawhide, catnip, hamster/guinea pig (still no bare pet/dog/cat). Makeup/skincare: makeup, cosmetics, serum, lotion, mascara, lipstick, Estee Lauder, perfume — not bare `cream`/`oil`. Classify skips these. Nightly ingest already bulk-No’s leftover pending rows that match.

**Limits:** Ingest image is on the box (Watchtower **2026-09-03 23:55Z**). Live Ingredient review queue stays until the next persist ingest (or `sudo docker exec yum4less-ingest npm run owner:reject-pending-junk-reviews`). Do not use bare `oven`, `air fry`, `pet`, `dog`, `cat`, `cream`, or `oil`.

**Evidence:** this session `npm test` **1236/1236** (217 files); junk + classify files include the Ninja countertop-oven title.

### 2026-09-03 — Dollar General Flipp ingest + food-desert dinners

**Theme:** Collect Dollar General ZIP circulars the same way as Food Lion/Walmart. Show directional sales even when dinners stay off. Rank dinners only in food-desert searches, with the **same** match/confidence/freshness floors — never margarine-as-butter and never a lowered threshold.

**Shipped:** Live Flipp-first ingest (`createDollarGeneralWeeklyAdIngestionClient`; scrape fallback if Flipp empty; fixture for CI). `031_promote_dollar_general_shopper_ranked.sql`. Food-desert gate: DG dinners only when no other shopper-ranked grocer is in the nearby set. Deals/sale ingredients can use weekly-ad rows without `recommendationEnabled`. Fail-loud DG flyer errors (Lidl stays fail-soft). Owner Check still warns without Kroger/Aldi/Publix/Food Lion/Walmart. Shopper copy: packaged/pantry **area circular**. `027` effect probe treats Lidl `map_context` as 027-applied so docker-style init does not undo `030`.

**Limits:** Live `031` applied **2026-09-03 23:59:42Z**. ZIP `23111` has other grocers, so DG dinners stay off there. Floors unchanged (`MIN_WEEKLY_AD_PROMOTION_MATCHES` = 3, confidence 0.45). Do not claim more dinners.

**Evidence:** this session `npm test` **1236/1236** (217 files); `npm run test:integration` **54/54** (17 files); `npm run test:e2e:ci` **32 passed** / **2 skipped** (first attempt: Playwright missing from Cursor sandbox cache; retry with user `ms-playwright`); `npm run build` pass (Next 15.5.21, inside e2e:ci). Postgres MCP `yum4less_dev`: Dollar General `shopper_ranked=true`, Lidl `map_context`. Semgrep MCP scanned temp copies — not a passed scan of repo paths.

### 2026-09-03 — Lidl map-context lock

**Theme:** Stop treating Lidl as dinner-ranked. ZIP-wide Flipp/hub circulars are not store-bound sale data. Do not keep unused store-finder scrape code.

**Shipped:** `030_demote_lidl_shopper_ranked.sql` — Lidl `shopper_ranked=false`, `settings_selectable=false`, `rollout_stage=map_context`, weekly-ad ingest still eligible. Fixture membership and `MEAL_PRICING_COMING_LATER_CHAINS` match. Shopper copy: Lidl is nearby context, not dinner totals. Fail-soft Lidl flyer errors. Removed Playwright store-finder probe files and `probe:lidl-*` scripts. Nightly Lidl ingest stays Flipp then hub HTML.

**Limits:** Live `030` applied **2026-09-03 23:59:42Z** — Lidl is `map_context`. Do not claim Lidl dinners. Walmart stays ranked with the same floors. No new Lidl scrape experiments.

**Evidence:** `npm test` **1226/1226** (214 files); `npm run build` pass. See [Verification snapshot](#verification-snapshot).

### 2026-09-01 — Lidl store-finder scrape probe

**Theme:** See whether Playwright can get Lidl flyer JSON by choosing a store (Select your store + ZIP), instead of dumping hub HTML.

**Shipped:** Owner probe `npm run probe:lidl-live-scrape` — Flipp vs Playwright store-finder. Discovers the current `esi-flyer` viewer from the live page (does **not** hardcode a dated flyer URL). Clicks Select your store (leaflet button or that label), types the probe ZIP, captures matching network JSON. Lidl hub URL is now Offers & Leaflets (`/c/offers-leaflets/s10092873`) because `/weekly-ads` **404s**. Helpers have unit tests. Nightly click-path is still **not** wired; ingest now at least fetches a live leaflets page instead of a 404.

**Limits:** Live Lidl.com is **not** a CI/Vitest gate. Probe writes nothing. CMS leaflets id can drift (homepage is a fallback). Store-result click still missed; ZIP was entered. Do not claim Lidl dinners from this probe.

**Evidence:** `npx vitest run` lidl store-finder + lidl ingest **12/12**. Live probe this session (ZIP `23111`, no persist): Flipp **137** offers; Playwright opened leaflets (HTTP 200), discovered `esi-flyer` for 8/26–9/1, captured grocery names in flyer JSON (chicken breast, watermelon). First probe run hit `/weekly-ads` 404 and clicked site search — selectors then tightened. Did not run full `npm test`, e2e, or integration. Semgrep MCP scan of these files returned no findings (also scanned an unrelated temp path).

### 2026-09-01 — Whole-ZIP ingest fence (code)

**Theme:** Nightly ingest admits the same grocery pins Owner Check lists: Census ZIP outline with a 26 mi safety cap, not outline ∩ density miles (packed 6).

**Shipped:** `INGEST_ZCTA_SAFETY_CAP_MILES` (rural 25+1) is the admission cap. `resolveIngestFenceForZip` auto-widens saved packed/urban miles. Check listing and `inIngestFence` use the same miles. Headline: `ingest ZIP outline (cap 26 mi)`. Shopper map stays pin + radius.

**Limits:** Live yum4less.com still uses density ∩ outline until Watchtower pulls. Neighbor-ZIP map pins still need those ZIPs Activated. One flyer per banner globally unchanged. Outline missing → 26 mi circle only.

**Evidence:** `npx vitest run` market-admission + owner ingest-markets **18/18**; owner-markets-panel **20/20** with catalog-sync panel files. Did not run full `npm test` (prior session worker-pool flakes). Did not run e2e (shopper flow unchanged) or integration (no schema/persist change).

### 2026-09-01 — Public weekly-ad status copy (sanitizer)

**Theme:** Privacy redact was blanking hyphenated English (`all-time`, `weekly-ad`) in weekly-ad status sentences, so curl JSON looked like a scrape failure.

**Shipped:** `redactInternalStoreIds` replaces known store/source ids (plus leftover OSM/SNAP/numeric catalog tokens), not every kebab-case word. Store ids stay stripped from public fields.

**Limits:** Live yum4less.com still serves the old sentence until Watchtower pulls. Does not change ingest, dinners, or floors.

**Evidence:** `npm test -- src/lib/public-api-response-sanitizer.test.ts` **7/7**. Full `npm test` this session hit unrelated worker-pool timeouts (5 failed / 1161 passed) — not treated as this slice. Did not run e2e (shopper flow unchanged). Semgrep MCP did not scan these files (unrelated temp ingest scripts).

### 2026-09-01 — Whole-ZIP ingest lock (owner)

**Theme:** Nightly ingest should cover the same grocery pins Owner Check lists (Census ZIP outline), so as more ZIPs are Activated, every store *in those ZIPs* has current sale data. Shopper map stays pin + radius.

**Shipped:** Decision only at the time. **Superseded same day** — fence is in code (ZIP outline, cap 26 mi). Live waits Watchtower.

**Limits:** Pins the shopper circle draws in a *neighbor* ZIP stay unpriced until that ZIP is also active. Keep a wide safety cap for giant rural ZCTAs (Check already uses 26 mi). One flyer per banner globally still fans out — far-apart storefronts in one huge ZIP stay a later per-market-flyer problem. Does not change shopper search, floors, or Flipp yield.

**Evidence:** owner confirmation this session. No tests.

### 2026-09-01 — Live Watchtower pull + `23220` first ingest (owner paste)

**Theme:** Confirm overnight cron and the 2026-08-31 `:homelab` recreate on TrueNAS.

**Shipped (ops):** Watchtower **2026-08-31 22:55Z** `Updated=2` — app `72c659133d11`, ingest `bfe5bf4124a1`. Postgres untouched (Up 40h). Site `https://yum4less.com/` `/` `/faq` `/terms` all **200**. `TRUST_PROXY_*` still `1`. 3am **2026-09-01** enqueued 6 jobs; worker drained **07:01–07:15Z**, all `succeeded`. `active_markets` **23111** + **23220** (both `packed` / 6 mi). Freshness **[OK] 571/571** ~5.2–5.4h. Kroger official **192** rows / **96** foods (second market, not more unique foods). No Lidl flyer. Walmart still **9** foods.

**Limits:** TheMealDB **dev test key**. Do **not** claim more dinners. `ingest.log` only shows enqueue (worker stdout is not that file). `chain_registry` key is `chain_id`, not `id`.

**Evidence:** owner TrueNAS paste this session.

### 2026-08-31 — Backlog honesty pass (owner lock)

**Theme:** Close stale open items that the repo and owner already treated as done; keep real later work named.

**Shipped (docs only):** Section H Home/Deals/Cook/Saved restyle, Q34/Q35 Settings-after-setup, ZIP-first visual rewrite, D7 owner browser verify, P1-8 Cook vs `marketBlocked`, GitHub MCP Pass 7 follow-up, Watchtower hourly-scan-as-pull-mechanism. SNAP CSV **parser + fixture path already exist**; nationwide USDA CSV on TrueNAS is ops-optional, not a missing feature. CI `db/ci/024_themealdb_rank_seed_clones.sql` stays for `yum4less_test` only (not `db/init/024`, not yum4less.com).

**Still open:** Flipp/scrape yield for Lidl / Walmart / Target / Dollar General / BJ's in `23111`; first `23220` 3am ingest confirm; M128/M151 scrape automation (robots/auto-pause/kill-switch — **not started**); Slice D matcher ingest; Wave 2 Q1 Publix map classifier; nationwide later (`threshold_profiles`, scrape TZ, banner-row split, organic waiting list); cuisine R11; cross-device Saved; accounts/Redis (same later horizon); Owner Check fragment lists; chain go/no-go (docs only, far future). **Never** coupon/checkout.

**Evidence:** code grep + Resume/Home/runbook cross-check this session. No product tests required.
...
| P1-8 Cook tab vs marketBlocked | **CLOSED** (2026-08-31) | Superseded by wizard + Section H restyle; Cook honest empty shipped in vision-gap sprint 1–6 |
...
| P1-8 Cook tab vs marketBlocked | **CLOSED** (2026-08-31) | Superseded by wizard + Section H; not a live Cook-tab bug |
...
| 2026-09-15 | **Wave 2 Q1 Publix map align implemented.** Q1=1B (2026-07-16): Publix locator is Settings-selectable catalog. Code removes Publix from `isMapContextCatalogStore` so map merge/suppress match that class. | **Active** (implements 2026-07-16 Q1=1B) |
| 2026-09-15 | **Bright Data MCP usage paused.** Local config may remain; agents must not use Bright Data for ingest, adapters, or routine probes until the owner unpauses. Free tier is research-only and not useful for Yum4Less coverage; paid aggregator/tier not planned for beta v1. Prefer Flipp + existing scrapers/APIs (aligns with free-stack go/no-go). | **Active** |
...
| 2026-09-14 | **Local ≠ live is not an open product task.** Laptop Compose and yum4less.com stay separate volumes. Do not point Cursor MCP at TrueNAS, dump live onto local daily, or rename DBs unless that confusion returns. Live catalog/ingest/migration numbers still require owner paste-back (§4.5). | **Closed** (open-task only; 2026-08-25 rule stays **Active**) |
| 2026-09-14 | **Unattended 3am + worker is standing ops, not an open proof task.** Live `ingest_jobs`: **15** consecutive nights **2026-08-31–2026-09-14**, each **6/6 succeeded**, enqueue ~07:00:03Z. Do not disable the TrueNAS worker Cron Job. Re-check only if a night is missing from `run_date`. | **Closed** (narrows 2026-08-31 one-night worker proof) |
| 2026-09-14 | **Obvious leftover grocery: one planner, two doors.** `planPendingReviewResolution` is SSOT. Live persist classify runs it after skip-table, alias, junk, fuzzy match, and simple-food auto-create (Yes → match/auto-create; No → skip `obvious-no`; skip/unsure → `/owner`). `/owner` **Clear obvious** POSTs `action: clear-obvious` (no apply on page load). CLI `owner:resolve-pending-reviews --apply` is the same write path. Canonical kebab ids, not brand slugs (`grill-seasoning` not `grill-mates-seasoning`). Does not rewrite already-accepted brand ids. Does not invent more dinners. | **Active** (narrows 2026-08-25 “peaches/steaks stay in review until auto-create widens”) |
| 2026-09-07 | **Database-owned data:** TypeScript holds behavior (fetch/parse/match/persist). Postgres holds operational facts: `active_markets`, `chain_registry`, store pins/`source_store_id`, `price_observations`, ingredients. Do not hardcode retailer store numbers (e.g. Target `1968`), silent ingest ZIPs, or banner rosters in adapters. Other ZIPs reuse the same adapter via locator + catalog persist. Secrets stay in env. Test fixtures belong in `yum4less_test` / `db/ci`, not production ingest modules. Rule: `.cursor/rules/yum4less-db-owned-data.mdc`. | **Active** (narrows nationwide de-hardcoding; does not migrate promotion floors) |
| 2026-09-03 | **Live `030`/`031` + Watchtower:** TrueNAS ingest migrate applied Lidl map-context and Dollar General ranked flags. App/ingest `:homelab` recreated 23:55Z. Watchtower still does not migrate by itself (3am ingest prep does; hand migrate optional). | **Active** (closes “live waits 030/031 migrate”; open-task reminder closed 2026-09-14) |
| 2026-09-03 | **Dollar General food-desert dinners + same floors:** Collect Flipp ZIP circulars (fan-out like Food Lion). Show directional sales without claiming store-shelf prices. Dinner totals only when **no other shopper-ranked grocer is nearby** and existing weekly-ad floors pass. Do not lower match/confidence floors. Do not match margarine as butter. Owner Check still warns if no supermarket banner is in the first look. Fail-loud ingest once ranked. **Live `031` applied 2026-09-03 23:59Z.** | **Active** (narrows 2026-07-09 Dollar Tree/DG queued; supersedes DG fail-soft-only) |
| 2026-09-03 | **Lidl map context until store-bound sales exist:** Lidl pins stay on the map. Lidl is **not** shopper-ranked and **not** used for dinner totals. Flipp ZIP circulars and generic hub/esi-flyer dumps are not a store-specific sale feed. Do not add Playwright store-finder, headed scrape, or MCP wrappers to chase Lidl dinners. Keep existing Flipp + hub ingest fail-soft. Revisit only if a licensed/store-bound source appears. | **Superseded** for ranking (2026-09-25 `036`, ZIP-ad note). Store-finder scrape stays off. |
| 2026-09-01 | **Lidl esi-flyer store-finder:** Use Playwright to discover the current `esi-flyer` viewer and click Select your store + ZIP. Capture flyer JSON, not brittle SVG HTML. Dated URLs (e.g. `weekly-ad-9-2-2026-9-8-2026-456102`) are resolved at run time, never hardcoded. Probe-only (`probe:lidl-live-scrape`) until it yields parseable offers — do not change nightly Lidl ingest on hope. | **Superseded** (2026-09-03: probes removed; Lidl not ranked) |
| 2026-09-01 | **Whole-ZIP ingest:** Nightly ingest admits the same grocery pins Owner Check lists (Census ZCTA ∩ 26 mi cap). Shopper map stays pin + radius. Neighbor-ZIP map pins need those ZIPs Activated. **On live ingest image** after Watchtower **2026-09-03 23:55Z**. | **Active** (supersedes 2026-08-29 density-mile ingest cap) |
| 2026-09-01 | **Live Watchtower + `23220` ingest:** `:homelab` recreate 2026-08-31 22:55Z; FAQ/Terms 200; 2026-09-01 worker 6/6 succeeded; both ZIPs active; freshness 571/571. Next product work remains Flipp/scrape yield. | **Active** (closes “wait for Watchtower pull” and “confirm first 23220 ingest”) |
| 2026-08-31 | **Backlog honesty:** Watchtower hourly scan = live `:homelab` pull (not a missing job). `23220` first ingest = 3am worker. TheMealDB dev test key stays known. Next product work = Flipp + scrape yield in `23111` (Lidl / Walmart / Target / DG / BJ's together). Section H + Q34/Q35 + ZIP-first layout + D7 verify + P1-8 **closed**. Cuisine chips (R11) were deferred until a cuisine facet existed — **closed 2026-09-15**. SNAP CSV code shipped; live USDA CSV is ops-optional. Cross-device Saved + accounts + Redis stay later together. Chain go/no-go stays in docs, far future. Owner Check fragments stay open. Coupon/checkout **never**. GitHub MCP Pass 7 follow-up **closed** (wrapper shipped 2026-07-16; in-session GitHub MCP tools available). CI seed-dinner clones stay test-only. | **Narrowed** (2026-09-15: R11 shipped) |
...
| 2026-08-31 | **Unattended ingest-worker night:** 3am enqueued 6 jobs; TrueNAS Cron Job drained them 07:01–07:09Z, all succeeded. Hash-skip may leave flyer `observed_at` from the prior drain. Extra ZIP `23220` next. | **Closed** (2026-09-14: 15-night `ingest_jobs` rollup) |
| 2026-09-03 | **Store lists = recognized banners only:** Settings, Owner Check, and density grocery counts omit convenience, bakeries, specialty, and independent OSM grocery leftovers. Keep known supermarket/club/dollar banners plus Target/Whole Foods. Map context pins stay on the map. | **Active** (narrows 2026-08-31 grocery-pin picker; 2026-08-29 Check omit) |
| 2026-08-31 | **Post-v1 shopper store gate:** Settings/wizard list grocery pins in radius (omit convenience/bakery/pharmacy/mini-mart). Not TypeScript `SHOPPER_RANKED_V1_CHAINS`. Dinner **estimates** still require `weeklyAdPromotionGatesPass` / official-API floors. No new adapters. Target OSM department_store kept by name only. | **Narrowed** (2026-09-03: independents/specialty/bakery not listed) |
...
| 2026-08-30 | **First live ingest-worker drain (manual):** TrueNAS `yum4less-ingest` ran `npm run ingest:worker` and drained 6 jobs with `[freshness] OK`. Fence 9 mi ∩ ZIP. Extra ZIP `23220` later Activated. | **Closed** (2026-09-14 unattended 15-night `ingest_jobs` proof) |
| 2026-08-29 | **Market admission:** Shopper radius stays around the GPS/ZIP pin — no H3 in the wizard. Check ZIP lists grocery/food-capable pins inside the Census ZCTA; omit convenience/bakeries; clubs food-only; Target/CVS/pharmacies and Giant/unknown = Needs you (no new adapters). **Ingest fence (2026-09-01):** ZIP outline ∩ 26 mi safety cap (same as Check). Density class still labels packed/urban. Check = inventory (no live Flipp). Activate books the ZIP even if flyer tests time out. MUST = grocery in fence. One flyer per banner globally, then fan-out; content-hash skip. Queue: 3am may enqueue; worker drains. Not in this Build: H3, Target/pharmacy scrapers, split 24h vs ad-week SLA. | **Active** (narrowed 2026-09-01: whole-ZIP ingest shipped in code) |
...
| 2026-08-27 | **Lidl shopper-ranked v1:** Settings-selectable and dinner estimates when weekly-ad promotion gates pass (same path as Food Lion). `SHOPPER_RANKED_V1_CHAINS` includes `lidl`; `MEAL_PRICING_COMING_LATER_CHAINS` stays empty. Lidl weekly-ad errors are fail-loud. `027` promotes `chain_registry` Lidl flags for Coverage honesty; ranking lists stay TypeScript. Coordinate sanity still required. | **Superseded** (2026-09-03 Lidl map-context lock) |
| 2026-08-27 | **Weekly-ad fail-soft for unranked chains:** cron succeeds if an unranked flyer errors. Fail-loud set is `chain_registry.shopper_ranked` (now includes Dollar General). Lidl flyer errors stay fail-soft after the 2026-09-03 map-context lock. `/owner` Check warns when the first look has none of the supermarket ranked banners; Activate is not blocked. | **Active** (narrowed 2026-09-03: DG fail-loud; Lidl fail-soft) |
...
| 2026-07-09 | **Dollar Tree / Dollar General (#18 queued):** Dollar Tree stays a separate onboarding investigation. Dollar General Flipp + food-desert dinners shipped 2026-09-03 (`031`) — not a lowered-floor or margarine-as-butter path. | **Narrowed** (DG dinners Active 2026-09-03; Dollar Tree still queued) |
...
| ~~2026-06-25~~ | ~~**Redesign deferred:** 5-tab shell, Deals, Cook session tab, Saved persistence, cuisine R11, pantry full UI, map-link-only shell move — after slices 1–5~~ | **Superseded** (D1–D6 + D7 + R11 shipped; cross-device Saved still paused) |
...
| `npm run build` (Whole Foods adapter) | 2026-09-22 | **Pass** (Next.js 15.5.25) |
| Semgrep CLI `p/default` (Whole Foods adapter/`034`) | 2026-09-22 | **0 findings** (1.165.0) |
...
| `npm test` (Mock1 tab icons) | 2026-09-21 | **1285/1285** pass (226 files) |
| `npm test` (shopper workflow SSOT) | 2026-09-17 | **1284/1284** pass (226 files) |
| `npm run test:e2e:ci` (shopper workflow branches) | 2026-09-17 | **37 passed** / **2 skipped** |
...
| `npm test` (Scale risk A client-trust API audit) | 2026-09-15 | **1274/1274** pass (225 files) |
| `npm test` (Wave 2 Q1 Publix map-context align) | 2026-09-15 | **1271/1271** pass (224 files) |
...
| `npm test` (Target weekly-ad ingest adapter) | 2026-09-15 | **1267/1267** pass (223 files) |
| `npm run test:integration` (Target `032`) | 2026-09-15 | **54/54** pass (17 files); applied `032_enable_target_weekly_ad_eligible.sql` |
| `npm run build` (Target adapter) | 2026-09-15 | **Pass** (Next.js 15.5.25) |
| `npm run test:e2e:ci` (Target adapter slice) | 2026-09-15 | **32 passed** / **2 skipped**. First attempt: Playwright Chromium missing from sandbox cache; retry after `npx playwright install chromium`. |
...
| TrueNAS `owner:resolve-pending-reviews --apply` | 2026-09-14 | **yes=190 no=14 skip=27 applied_ok=204 applied_fail=0** (queue had been 231 pending). |
...
| TrueNAS `owner:reject-pending-junk-reviews` (`44e4e16` leftover junk) | 2026-09-10 | **scanned=902 rejected=285 remaining=617** (prior heal on same 902 was rejected=0) |
| `npm test` (Halloween/GM/drink leftover junk skip) | 2026-09-10 | Junk+classify **10/10**. Full suite **1237/1237** (217 files). |
...
| `npm test` (store-list omit: specialty/bakery/independents) | 2026-09-03 | **1237/1237** pass (217 files) |
| `npm test` (Dollar General Flipp + food-desert dinners + 031) | 2026-09-03 | **1236/1236** pass (217 files) |
| `npm run build` (DG ingest + desert gate; e2e:ci) | 2026-09-03 | **Pass** (Next.js 15.5.21) |
| `npm run test:integration` (`031` DG registry + `027` Lidl probe supersede) | 2026-09-03 | **54/54** pass (17 files). First targeted re-run 5/5 after 027 probe treated Lidl `map_context` as applied; full suite green. |
| `npm run test:e2e:ci` (Settings/map/trust; 23111 ranked stores are not DG) | 2026-09-03 | **32 passed** / **2 skipped**. First attempt: Playwright missing from Cursor sandbox cache. Retry with user `ms-playwright`. |
| Postgres MCP (`yum4less_dev` — local Docker, not TrueNAS) | 2026-09-03 | Dollar General `shopper_ranked=true`, `settings_selectable=true`, `rollout_stage=ranked`, `sale_discovery_strategy=flipp`. Lidl `shopper_ranked=false`, `rollout_stage=map_context`. |
| Semgrep MCP (DG ingest / desert-gate files) | 2026-09-03 | Scanned unrelated temp copies — **not** a passed scan of these files |
| `npm test` (Lidl map-context demotion + migration 030) | 2026-09-03 | **1226/1226** pass (214 files) |
| `npm run build` (deleted store-finder modules; hub URL move) | 2026-09-03 | **Pass** (Next.js 15.5.21) |
...
| Postgres MCP (`yum4less_dev` — local Docker, not TrueNAS) | 2026-08-31 | Six banners `shopper_ranked=true`: kroger, aldi, publix, food-lion, lidl, walmart. **`030` not yet applied locally.** |
...
| Homelab deploy + exposure | **Mostly closed:** TrueNAS Custom App + ingest + Watchtower + Cloudflare Tunnel + 3am two-night + unattended worker night + backup drill. Remaining: confirm `23220` 3am ingest; Watchtower hourly pull is the live update (owner 2026-08-31). M128 scrape automation still not started. |
| Walmart / Lidl / Target / Dollar General / BJ's **sale yield** | Adapters exist for Lidl + Walmart (Flipp + scrape). Yield is thin (no Lidl flyer in `23111`; Walmart junk-heavy). Target / DG / BJ's stay map/context until Flipp/scrape work is strong enough. Same coverage bucket — not coupon/checkout. |
| Redis / platform rate limits | **Later with multi-instance.** In-process per-IP limits already ship (`src/lib/rate-limit.ts`). One TrueNAS app container is enough today. |
| User accounts | Explicitly out until Redis-scale traffic; same pause as cross-device Saved. **Never** coupon/checkout. |
...
| **Scale risk A — Client-trust audit across all public API routes** | **CLOSED (2026-09-15)** — Cross-route audit; pantry spoof tests; analytics client-stamped dims documented non-authoritative. |
...
| **General locator-vs-OSM dedupe across all v1 chains (Option A)** | **IN PROGRESS** — Slice 1–6 **CLOSED** (2026-07-11); Slice D matcher **shipped in repo** (2026-09-22) | Alias-graph. **5c:** ingest self-alias + allowlisted Aldi→OSM pointer. **Slice D:** catalog OSM↔official proximity matcher writes **provisional** aliases after map-catalog. Flags + `AUTO_CONFIRM` still **OFF**; shopper expand still **OFF** until review. Allowlist stays for exact-pointer confirmed links. |
...
| 4 | Universal store reconciliation | **Slice 1–6 CLOSED**; Slice D matcher **in repo** (provisional; AUTO_CONFIRM OFF) | Med | Low–Med | Option A slices |
...
| 2026-09-17 | Shopper workflow SSOT + e2e branch gaps | [Shopper workflow SSOT](da871ddb-2a23-4a68-88f4-27b7656d9c7c) |
...
| 2026-09-05 | TrueNAS ingest health + store-list omit check | [Dollar General food desert](57902417-8553-43df-8073-79b429b9c8bb) |
| 2026-09-03 | Store lists omit specialty/bakery/independents | [Dollar General food desert](57902417-8553-43df-8073-79b429b9c8bb) |
| 2026-09-03 | Dollar General Flipp ingest + food-desert dinners | [Dollar General food desert](57902417-8553-43df-8073-79b429b9c8bb) |
