...
6. ~~**Deferred D1–D6** — 5-tab shell, theme tokens, ingredient gate/chips, map-as-link overlay, session pantry UI~~ **done (2026-06-25)** — cuisine chips (R11) still deferred (filter dinners by cuisine/ethnic tags; hidden until recipes have a cuisine facet in the DB — today they only have generic `tags[]` / `dietary_tags[]`). Device-local Saved meals shipped 2026-08-20; **cross-device Saved stays paused** (needs accounts; same horizon as Redis).
...
**Later (when reprioritized):** chain go/no-go on the free stack (OSM / Flipp / scrape; no paid aggregator) — **keep in docs, not near-term**; GYAM Cloudflare hostname on same tunnel; Saved tab **cross-device** persistence (paused with accounts); cuisine chips (R11 — see Next above); SNAP nationwide USDA CSV on TrueNAS (parser + fixture path **shipped**; live CSV path is ops-optional); banner-per-row with shared family adapters (not 20 copied Kroger clients). Owner Check fragment lists: research, stay open. New ranked adapters stay open. **Never** coupon/checkout. User accounts stay out until Redis-scale traffic. BJ's / Target ranked pricing groups with remaining Flipp+scrape yield — not a separate v1-out forever.
...
Saved tab **cross-device** persistence stays paused (device-local Saved shipped). Cuisine DB/tags (**R11**) remain deferred until recipes have a cuisine facet. Mockup layout polish from the old Settings-first shell is closed (wizard + Section H).
