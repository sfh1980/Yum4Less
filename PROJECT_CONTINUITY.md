...
- **Store-list omit on live app (2026-09-14):** `yum4less-app:homelab` created **19:43:47Z**. Grep: `fas mart` in `/app/.next/server/app/page.js` and shopper chunks. Closes “hard-refresh Settings to prove omit code is on the box.” Map may still show gas/bakery/Joe’s pins. Ingest image not inspected. Do **not** claim more dinners.
...
- **Obvious grocery ingest + Clear obvious (2026-09-14, local):** Persist ingest files leftover grocery with the same planner as `/owner` **Clear obvious**. Live apply earlier today **applied_ok=204**. Skip leftovers still need a human until this image is on the box. Do **not** claim more dinners.
...
- **Homelab still open (do not fold into closed):** TheMealDB **dev test key** on live ingest (known). Website `robots.txt` for `/owner` is still a separate SEO leftover (not scrape compliance). Watchtower pulls **closed** (2026-08-31 22:55Z and **2026-09-03 23:55Z** `Updated=2`). Extra ZIP `23220` first ingest **closed** (2026-09-01 worker 6/6 `succeeded`; freshness **571/571**).
...
**Limits:** Unsure sides/mystery sauces still need a human. Does not rewrite old brand ids. Does not invent more dinners. Needs ingest image pull for live classify; app image pull for the button.

**Evidence:** this session `npm test` **1246/1246** (218 files). `npm run build` pass (Next 15.5.25; `/api/owner/ingredient-reviews` + `/owner`). Targeted planner + classify + owner route + console **29/29** after garden-salad auto-create and Grill Mates → generic `grill-seasoning`. `npm run test:e2e:ci` not run (`/owner` is key-gated; shopper flow unchanged). Semgrep MCP timed out on these files — not a passed scan. Live classify + **Clear obvious** wait for Watchtower ingest **and** app images.
