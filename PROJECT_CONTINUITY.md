...
**Nationwide de-hardcoding:** **A** (no silent `23111`), **B1** (`active_markets` + `zip_geocode_cache` / `025`), **B2** (`chain_registry` + `store_coverage` + `/owner` Coverage / `026`), and **C** (`/owner` Markets ZIP check + activate) are **current**. Plan → [`docs/audits/de-hardcoding-nationwide-db-driven-plan-2026-08-12.md`](docs/audits/de-hardcoding-nationwide-db-driven-plan-2026-08-12.md). Expansion is more owner-activated ZIPs. **Owner-chosen ranked ingest anchor is ZIP `23111`** (explicit `active_markets` row — not a code default). Ranked dinners still only where Kroger-family, Aldi, Publix, or Food Lion exist. Weekly-ad ingest now **fail-soft** on Walmart / Lidl / Dollar General flyer errors; those four ranked banners stay fail-loud. Chain go/no-go stays later (free Flipp/OSM/scrape only; trial and error). Shopper ranking lists stay TypeScript until a later dual-run cutover. **Live ingest overlay is off** as of owner paste-back **2026-08-27**: cron/manual scheduled ingest visited **1 ZIP** from `active_markets`. Keep only `23111` until unattended 3am exits 0 with overlay off, then prefer another four-chain ZIP.
...
**Limits:** Fail-loud is shopper v1 lists, not `WEEKLY_AD_RANKED_PRICING_CHAINS` (Lidl/Walmart still persist for rehearsal). Provider price-sync exit is unchanged (any persist fail still fails that script). Extra ZIPs still wait for unattended 3am cron with overlay off. Local Docker ≠ yum4less.com.
...
|------|----------|--------|
| 2026-08-27 | **Weekly-ad fail-soft for unranked chains:** cron succeeds if Walmart / Lidl / Dollar General flyer error or persist fail. Kroger / Aldi / Publix / Food Lion stay fail-loud (error or persist fail → non-zero). Fail-loud set is `SHOPPER_RANKED_V1_CHAINS`, not `WEEKLY_AD_RANKED_PRICING_CHAINS`. `/owner` Check warns when the first look has none of those four banners; Activate is not blocked. Not go/no-go. Does not rank Walmart. | **Active** |
