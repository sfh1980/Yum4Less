...
- **Dollar General Flipp ingest + food-desert dinners (2026-09-03, repo):** Live Flipp-first client (scrape fallback if Flipp empty); fixture for CI. Deals/sale chips can show DG weekly-ad rows without claiming dinners. Dinner totals only when no other shopper-ranked grocer is nearby and the same match/confidence/freshness floors pass. Copy: directional packaged/pantry **area circular**. Owner Check still warns without a supermarket banner. Local `yum4less_dev` has `031` applied. Live yum4less.com waits ingest migrate `030`+`031` + Watchtower. Do **not** claim more dinners in `23111` (other grocers nearby).
- **Market admission (unattended worker 2026-08-31):** Shopper wizard is still GPS pin or ZIP → map pin. **Code now** admits ingest by **ZIP outline (cap 26 mi)**; live images still use density miles ∩ outline until Watchtower. One flyer per banner, then fan-out. Hash skip can leave weekly-ad `observed_at` from Sunday. `23220` first ingest **2026-09-01**. No Lidl flyer. TheMealDB still logs **dev test key**. Do **not** claim more dinners. **Keep the Cron Job Enabled.** Next coverage work is Flipp/scrape yield for Walmart / Target / BJ's in `23111`.
...
- **Homelab still open (do not fold into closed):** TheMealDB **dev test key** on live ingest (known). Website `robots.txt` for `/owner` is still a separate SEO leftover (not scrape compliance). Watchtower pull **closed** (2026-08-31 22:55Z, Updated=2). Extra ZIP `23220` first ingest **closed** (2026-09-01 worker 6/6 `succeeded`; freshness **571/571**).
...
**Limits:** Live queue stays until ingest image + Watchtower, then the next persist ingest (or `npm run owner:reject-pending-junk-reviews` on the NAS). Do not use bare `oven`, `air fry`, `pet`, `dog`, `cat`, `cream`, or `oil`.
...
**Limits:** Live yum4less.com still needs ingest migrate `030`+`031` and Watchtower. ZIP `23111` has other grocers, so DG dinners stay off there. Floors unchanged (`MIN_WEEKLY_AD_PROMOTION_MATCHES` = 3, confidence 0.45). Do not claim more dinners.
...
**Limits:** Live yum4less.com still has `027` Lidl ranked until ingest migrate applies `030` and Watchtower pulls app+ingest. Do not claim Lidl dinners. Walmart stays ranked with the same floors. No new Lidl scrape experiments.
...
| 2026-09-01 | **Whole-ZIP ingest:** Nightly ingest admits the same grocery pins Owner Check lists (Census ZCTA ∩ 26 mi cap). Shopper map stays pin + radius. Neighbor-ZIP map pins need those ZIPs Activated. **Shipped in code** 2026-09-01; live waits Watchtower. | **Active** (supersedes 2026-08-29 density-mile ingest cap) |
