...
**Hosting:** Self-hosted TrueNAS SCALE — **Custom App `yum4less` (`db`+`app`+`ingest`) healthy**; **Watchtower** sibling up; **Cloudflare Tunnel** live at **`https://yum4less.com/`** (LAN `:3000` remains). Images on **`:homelab`**. Wizard shopper UI is on **`master`** after this slice’s push; live `yum4less.com` updates only after green `publish-image` / `publish-ingest-image` and Watchtower pull. Rollback: **`backup/settings-ui-plus-catalog`** (`38bad3a`, Settings-first + catalog) or older **`backup/pre-catalog-expansion-live`** / **`:800c450`**. Watchtower still does **not** migrate. Live ledger includes **`024`** (`applied_at` **2026-08-24 07:00:02Z**), **`025`** and **`026`** (`applied_at` **2026-08-27 00:04:01Z**, owner paste-back **2026-08-26**). Future `db/init` files still need an operator migrate.
...
**Next (nationwide de-hardcoding — ordered):** ~~A no silent ZIP~~ **done** · ~~B1 `active_markets` + ZIP cache~~ **done** (TrueNAS `025`) · ~~B2 `chain_registry` + Coverage~~ **done** (TrueNAS `026`) · ~~C `/owner` Markets ZIP activate~~ **done** · ~~weekly-ad fail-soft for unranked chains~~ **done** · ~~Lidl shopper-ranked v1~~ **done in repo** (TrueNAS still needs `027` + app/ingest image). Extra ZIPs wait for unattended **3am** cron EXIT 0 with overlay off, then prefer another ranked-chain ZIP (e.g. `23220`). Organic shopper-ZIP waiting list, threshold profiles, scrape TZ, Slice D identity, `chain_registry` dual-run cutover, and new ranked adapters stay later. Do not cut shopper ranking over to `chain_registry` in this C.

**Later (when reprioritized):** chain go/no-go on the free stack (OSM / Flipp / scrape; no paid aggregator); Section H restyle of Home/Deals/Cook/Saved + Q34/Q35 compact Settings; confirm unattended 3am ingest cron + Watchtower first hourly scan; backup/restore drill on TrueNAS; GYAM Cloudflare hostname on same tunnel; Saved tab persistence; cuisine chips (R11); optional SNAP CSV ingest; true client-side ranking from API-sent chain ids (dual-run CI check is current; DB-wins in the browser is later).
...
**Limits:** Live Walmart Flipp/scrape is often thin — estimates appear only when floors pass; otherwise map context with an honest reason. TrueNAS still needs `027` + `028` plus app **and** ingest image pulls (Watchtower does not migrate). Extra ZIPs still wait for unattended 3am with overlay off. Local Docker ≠ yum4less.com. Client Settings still cannot query Postgres (dual-run, not DB-wins in the browser).
