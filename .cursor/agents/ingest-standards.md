...
- **Per-chain scrape compliance guard (M128/M151):** robots.txt before scheduled scrapes; detect block signals (403, CAPTCHA, challenge pages); auto-pause that chain's ingest without affecting others; manual per-chain kill switch (`YUM4LESS_DISABLE_INGEST_*` env vars) always available independent of automation. Full automation ships with homelab deploy slice — until then: log + owner pause on block signals.
